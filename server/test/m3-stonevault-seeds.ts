/**
 * M3 — Saat-Test fuer die Modul-Kits `DG_StoneVault` und `DG_RockVault`.
 *
 * Seit F4 laeuft JEDE der neun Pruefungen fuer BEIDE Kits (s. `KITS`
 * weiter unten). Wo im Folgenden `StoneVault…` steht, ist der Namensstamm
 * gemeint, den `modulPraefix()` am Kit selbst abliest.
 *
...
/**
 * Die Kits, die dieser Test misst.
 *
 * Seit F4 sind es ZWEI: `DG_StoneVault` in Ziegeln und die Fels-Ableitung
 * `DG_RockVault` (`rockVariant()` in `shared/src/eigeneDungeons.ts`).
 * Beide durchlaufen dieselben neun Pruefungen, und zwar VOLLSTAENDIG und
 * nicht abgekuerzt: Dass die Kit-Erklaerung abgeleitet ist, belegt
 * `shared/test/kit-ableitung.ts` — dass daraus auch ueber 40 Saaten
 * derselbe dichte Grundriss entsteht, ist eine Aussage ueber den
 * GENERATOR und keine ueber die Ableitung. Waere hier nur das Stammkit
 * gemessen, bliebe dieser Test gruen, waehrend das Fels-Grab Loecher hat.
 */
const KITS = ['DG_StoneVault', 'DG_RockVault'];
...
/**
 * Der Namensstamm der Module dieses Kits — `StoneVault` bzw. `RockVault`.
 *
 * ABGELESEN am Eingangsraum und nicht aus dem Kitnamen gerechnet: Beide
 * Kits heissen `DG_…Vault`, die Module aber nicht `DG_…` — eine Ableitung
 * aus dem Kitnamen waere eine zweite Namenskonvention neben der
 * bestehenden, und die stimmte genau so lange, bis jemand ein Kit anders
 * nennt als seine Module. `entrance` ist dabei eine Rolle, die es in
 * jedem Modulkit genau einmal gibt (s. `StoneVaultEntry`).
 */
function modulPraefix(def: { rooms: readonly RoomDef[] }): string {
  const eingang = def.rooms.find((r) => r.entrance);
  if (!eingang || !eingang.name.endsWith('Entry')) {
    throw new Error(`Kit ohne Eingangsmodul '…Entry' — der Namensstamm der Module ist nicht ablesbar.`);
  }
  return eingang.name.slice(0, -'Entry'.length);
}

/**
 * Alle neun Pruefungen fuer EIN Kit.
 *
 * Der Namensstamm der Module (`StoneVault…` / `RockVault…`) wird am Kit
 * ABGELESEN und nicht angenommen — s. `modulPraefix`. Ein Test, der ihn
 * aus dem Kitnamen raet, misst seine eigene Namenskonvention mit.
 */
function pruefeKit(KIT_NAME: string): void {
  console.log(`\n=== ${KIT_NAME} ===\n`);
  /** Wie `check`, nur dass die Meldung sagt, WELCHES Kit gemeint ist. */
  const pruefe = (name: string, cond: boolean, detail = ''): void => check(`${KIT_NAME} ${name}`, cond, detail);
  ...
  /** Namensstamm der Module dieses Kits — s. `modulPraefix`. */
  const P = modulPraefix(def);
   ...
   */
  const ZELLVARIANTEN = [
    `${P}Corridor`, `${P}Corner`, `${P}Junction`,
    `${P}Hall`, `${P}HallLarge`, `${P}HallLong`,
    `${P}HallGrand`, `${P}HallVast`,
  ];
  /** Varianten, die den Eindruck eines Gangsystems tragen (s. Pruefung 7) — die Halle zaehlt hier nicht: sie ist ein Saal, kein Gang. */
  const GANGVARIANTEN = [`${P}Corridor`, `${P}Corner`, `${P}Junction`];
  ...
  const SAAL_GRENZEN: readonly (readonly [string, number])[] = [
    [`${P}Hall`, 8],
    [`${P}HallLarge`, 8],
    [`${P}HallLong`, 5],
    [`${P}HallGrand`, 4],
  ...
  for (const seed of SEEDS) {
    ...
    pruefe(`seed ${seed}: Determinismus`, JSON.stringify(layout1) === JSON.stringify(layout2));

    const { all } = findOverlaps(layout1);
    pruefe(`seed ${seed}: keine Überlappung`, all.length === 0, all.slice(0, 3).join('; '));
    ...
    if (layout1.doors.some((d) => d.prefabName === `${P}Arch`)) archGesetzt = true;
    ...
    const nichtWand = layout1.rooms.length - (namen.get(`${P}Wall`) ?? 0);
  ...
  for (const variante of ZELLVARIANTEN) {
    ...
    pruefe(`Variante ${variante} kommt über die 40 Seeds mindestens einmal vor`, kommtVor);
  }
  ...
  for (const [saal, grenze] of SAAL_GRENZEN) {
    ...
    pruefe(`${saal} in mindestens ${grenze} von 40 Seeds gesetzt`, n >= grenze, `${n} Seeds`);
  }
  ...
  pruefe(`Raumzahl im Schnitt > 10`, avgRooms > 10, `Schnitt ${avgRooms.toFixed(2)}`);
  pruefe(`mindestens ein Arch-Torbogen über alle Seeds gesetzt`, archGesetzt);
  ...
  console.log(`Halle (${P}Hall) gesetzt in ${seedsMitHalle} von 40 Seeds`);
  ...
  const gesamtNichtWand = namensverteilungJeSeed.reduce(
    (sum, namen) => sum + [...namen.entries()].reduce((s, [name, n]) => (name === `${P}Wall` ? s : s + n), 0),
    0
  );
  console.log(
    `Treppen (${P}Stairs, je ${MODUL_ZELLE_HOEHE_M} m Ebenenhoehe, Huelle ueber 2 Ebenen = ${2 * MODUL_ZELLE_HOEHE_M} m): ` +
  ...
  pruefe(`${P}Stairs kommt über die 40 Seeds mindestens in 5 Seeds vor`, seedsMitTreppe >= 5, `${seedsMitTreppe} Seeds`);
...
for (const kit of KITS) pruefeKit(kit);
