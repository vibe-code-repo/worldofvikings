...
**Quellen:** der erste Prototyp (Browser-Game), dessen Asset-Ordner und der Referenzserver
...
>
> **Teil 2 (Weltgenerierung des Referenzservers) und die Status-Updates zur Mathematik gelten unverändert.**
> Weltgeometrie, Heightmap, Perlin, Seed-Offsets, Biom-Formeln, Flüsse, FastNoise/Ashlands —
> das ist gegen den Referenzserver golden verifiziert und bis heute die Grundlage von
> `shared/src/worldgen`. Wer daran etwas ändern will, liest hier nach.
>
> **Teil 1 und alles, was am Extraktions-Export hängt, ist seit dem 16.08.2026 Geschichte.**
> Der Export ist gelöscht (11.869 Dateien, 5,1 GB). Es gibt keine 7.463 GLBs mehr, keine
> 4,8 GB, keine 2.639 leeren PNG-Stubs, keinen Fremd-Asset-Ordner. Die Welt besteht aus
...
3. **Die Original-Skalierung fehlt komplett**: Im Vorbild bekommt jeder Baum einen Zufalls-Scale (Tanne im Schwarzwald: **2,0–2,5×** → 22–27 m hoch!). Der Browser-Server spawnt Bäume ohne Rotation und ohne Scale.
...
5. **Die gute Nachricht**: Der Referenzserver enthält die komplette Original-Weltgenerierung (1:1-Port des WorldGenerator, Game-Version 0.221.10, WorldGen-Version 2) — inklusive aller Zahlenwerte, Unity-exaktem Perlin, RNG und geparsten Datenpaketen (vegetation.pkg, features.pkg, prefabs.pkg). Das ist eine vollständige Port-Blaupause.
...
- **Assets**: `AssetManager` lädt `/assets/models/<name>.glb` (GLTFLoader) + PNG-Sprites, mit Canvas-Platzhaltern bei Fehlschlag. Vite serviert den externen Asset-Ordner unter `/assets` und proxyt `/ws` → Port 2456.
...
Die tatsächliche Pipeline (dokumentiert im Log von `tools/asset-export/`):

- **Extraktionswerkzeug 1.3.14.0** (GUI), zwei Läufe am 22.07.2026.
- Quelle: lokaler Client-Export (Linux-Client 0.221.12).
- Einstellungen: PNG-Export, DirectExport, Dummy-Shader, StaticMeshSeparation. **Kein Skalierungs-Setting — der Export erfolgt in Unity-Metern mit Original-Prefab-Hierarchie als glTF-Node-Baum.**
...
- Verifiziert: der `models/`-Ordner des Prototyps (7463 GLBs) ist eine 1:1-Kopie des Ordners `PrefabHierarchyObject/` in `tools/asset-export/` (MD5-geprüft bei FirTree.glb).

## 1.3 Asset-Bestand (Asset-Ordner des Prototyps)
...
| `audio/` | nur eigenes Material (MP3); die 3318 .ogg des Exports sind gelöscht — keine Fremdaufnahmen |
...
Referenzen: Werkbank 3,72×1,85×2,10 m, Holzportal 4,23×3,29×1,18 m, Truhe 1,64×0,79×1,13 m — alles plausible Meter-Maße.
...
- **Keine pro-Instanz-Zufallsskalierung**: Im Vorbild bekommt Vegetation über das ZoneSystem einen Zufalls-Scale (ZDO-Vec3-Member `"scale"`). Hier spawnt der Demo-Spawn mit Identitäts-Rotation und ohne Scale-Member (`server/src/WovServer.ts` Z. 137–146). Das Flag `SYNC_INITIAL_SCALE` existiert (`shared/src/types.ts` Z. 28), wird aber nirgends benutzt.
...
- **Serverseitig: keine Weltgenerierung.** Kein `server/src/world/`-Verzeichnis. Stattdessen `spawnDemoWorld()` (Z. 137–189): ~50 feste ZDOs um den Ursprung — Basis-Camp, **24 Bäume im Ring** (Rotation über 5 Arten, Abstand 25–49 m), Felsen, Pickables, Items, 4 Kreaturen. Alle auf y=0, Identitäts-Rotation. Der Seed `KxSYuZquuw` aus `server/data/server.yml` wird nirgends ausgewertet.
...
| Server-Orchestrator (528 Z.) | 30-Hz-Tick, Weltzeit, ZDO-Delta-Sync (50 ms, viewRadius 3 Zonen), Chat, server-autoritative Bewegung |
...
# Teil 2: Weltgenerierung (Referenzserver)

Analysiertes Repo: der Referenzserver — Port des originalen WorldGenerator, Ziel-Game-Version **0.221.10**, WorldGen-Version **2**. Die `.pkg`-Daten wurden vollständig binär geparst.

## 2.1 Weltgeometrie / Grundkonstanten

| Konstante | Wert |
|---|---|
| Zone-Größe | **64 m × 64 m** (`UNITS_PER_ZONE = 64`) |
| Heightmap-Vertices pro Zone | **65 × 65** |
| **Wasser-Level (Ozean-Y)** | **30.0** (`WATER_LEVEL = 30`) |
| Welt-Radius (Logik) | 10000 m |
| Weltrand (Wasserkante) | 10500 m |
| Zonenradius der Welt | 164 Zonen (Durchmesser 328) |
| Aktiver Generierungsradius um Spieler | je 2 Zonen (NEAR/DISTANT) |

**Einheiten-System**: Alle Biom-Höhenfunktionen liefern ~0..0,5 und werden in `GetBiomeHeight` mit **× 200** skaliert. Basishöhe 0,15 entspricht also exakt dem Wasser-Level 30 m. Ocean = Basishöhe ≤ 0,02.

Zone-Koordinaten: `WorldToZonePos` = `floor((p + 32)/64)`, `ZoneToWorldPos` = `zone*64` (Zonenzentrum).
...
- Alle 4 Eckbiome gleich → direkt `GetBiomeHeight`; sonst **bi-lineares Blending der 4 Eckhöhen**, `tx = rx/64`, `ty = ry/64`, bei aktivem `worldBlendBiomesSmoothStep` (Default **true**) durch `SmoothStep(0,1,t)`. Mistlands-Maske wird in `vegMask` (64×64) gespeichert.

### Perlin-Noise (Unity-exakt)
Klassischer Perlin mit **fester Permutationstabelle** (2×256 Einträge, beginnt `151,160,137,91,90,15,...`), Fade `6t⁵−15t⁴+10t³`, 16-Gradienten, Rückgabe **`(res + 0.69) / 1.483`** → [0,1]. Validierungswerte gegen Unity: `data/tests/perlin_values.txt`. Hilfsfunktionen: `LerpStep`, `SmoothStep`, `MathfLikeSmoothStep` (`t = −2t³+3t²`), `Fbm`, `Remap`, `BlendOverlay`.

### Seed-Offsets
Aus `seed = getStableHash(seedName)` (djb2-Variante: 5381-Start, `num = (num<<5)+num ^ c`, Ergebnis `num + num2*1566083941`) wird ein Unity-kompatibler **xorshift-RNG** (Multiplikator `0x6c078965`, `next_float = (next_int() & 0x7FFFFF) * 1.192093e-7f`) gezogen: `offset0..offset4` je ∈ [−10000, +10000], plus `riverSeed`, `streamSeed`. Alle Noise-Felder addieren **+100000** auf die Koordinaten (Perlin nutzt `abs()`).

### GetBaseHeight
Eingang `dwx = wx + 100000 + offset0`, `dwy = wy + 100000 + offset1`:
```
...
### Biom-Höhenformeln (jeweils +100000 + `offset3`; Ergebnis ×200)

- **Meadows** (`GetMeadowsHeight`): `n = P(0.01)·P(0.02) + P(0.05)·P(0.1)·n·0.5`; `h = base + n·0.1`; Absenkung über 0,15: `h -= (h−0.15)·(1−Clamp01(base/0.4))·0.75`; dann `AddRivers`; `+ P(0.1)·0.01 + P(0.4)·0.003`.
- **Plains**: identisch zu Meadows.
- **BlackForest** (`GetForestHeight`): `h = base + n·0.1` (**ohne** die 0,15-Absenkung); `AddRivers`; `+ P(0.1)·0.01 + P(0.4)·0.003`.
- **Swamp** (`GetMarshHeight`): Start **0,137**; `+ P(0.04)·P(0.08)·0.03` (hier +100000 **ohne** offset3!); `AddRivers`; Feinschliff wie oben.
- **Mountain** (`GetSnowMountainHeight`): `h = base + (base−0.4) + n·0.2`; `AddRivers`; `+ P(0.1)·0.01 + P(0.4)·0.003 + P(0.2)·2.0·BaseHeightTilt` (Tilt = Summe |Δh| über 4 Nachbarn ±1 m).
- **DeepNorth**: `h = base + max(0, base−0.4) + n·0.2`; **`h *= 1.2`**; `AddRivers`; Feinschliff.
- **Mistlands**: `n2 = P(0.014)·P(0.028) + P(0.021)·P(0.035)·n2·0.5`; `n2>0 → pow(n2,1.5)`; `h = base + n2·0.4`; `AddRivers`; Terrassierung `h = Lerp(h + P(0.4)·0.002, ceil(h·400)/400, n3)` mit `n3 = Clamp01(n2·7)`.
- **Ocean**: nur `GetBaseHeight` (kein AddRivers, da `worldRiverAffectsOcean=false`).
- **Ashlands**: komplexester Modus — **aktiv ist "modern noise"** (`worldAshlandsModernNoise=true`) mit **FastNoise** (seed=0, FBM, Cellular Euclidean, Jitter 0,45; Port des originalen FastNoise), 5 Cellular-Oktaven, Overlay-Blends, Lava-Maske, Senken. Legacy-Modus existiert für alte Welten.

### Flüsse & Seen
- **Seen**: Punktraster 128 m; `GetBaseHeight < 0.05` → Merge mit Radius 800.
- **Flüsse**: verbinden Seen, maxDistance 2000 (Fallback 5000); Breite 60–100 m; `curveWidth = Länge/15`, `curveWavelength = Länge/20`; Kurvenoffset `d = sin(t)·sin(t·0.63412)·sin(t·0.33412)·curveWidth`.
- **Bäche**: 3000 Versuche; Start 26–31 m, Ende 36–44 m Höhe, Länge 80–200 m, Breite 20 m.
- **AddRivers**: Gewicht `w = max(1 − dist/width)`; Zielhöhen `Lerp(0.14,0.12,t)` / `Lerp(0.139,0.128,t)` mit `t = LerpStep(20,60,width)`; schneidet Flussbetten in die Höhe.

### Finale Höhenabfrage
- Die Heightmap-Regenerierung kopiert Base-Heights 1:1; Ozeantiefe an Ecken = `max(0, 30 − cornerHeight)` bilinear.
- `GetWorldHeight`: bei `worldBilinearHeightSampling=false` (**aktiv**) **nearest vertex**; sonst Triangle-baryzentrisch.
- Vegetation nutzt **`GetWorldHeightRaycast`** (Möller-Trumbore auf 2-Triangle-Quads) via `GetGroundData`.

## 2.3 Biom-System

**Enum**: Meadows=1, Swamp=2, Mountain=4, BlackForest=8, Plains=16, AshLands=32, DeepNorth=64, (128 unbenutzt), Ocean=256, Mistlands=512. BiomeArea: Edge=1, Median=2, Everything=3.

**Entscheidungslogik** `GetBiome(wx, wy)` strikt in dieser Reihenfolge, mit `num = WorldAngle·100`, `WorldAngle = sin(atan2(wx,wy)·20)`:
...
**BiomeArea**: 9 Punkte im ±64-m-Raster; alle gleich → Median, sonst Edge.

**Wald-Faktor**: `GetForestFactor = Fbm(pos·0.004, 3 Oktaven, Lacunarity 1.6, Gain 0.7)`; `InForest` ⇔ Faktor < 1,15.

## 2.4 Vegetation / Feature-Platzierung

### Vegetation-Loop (`PopulateFoliage`)
Pro Zone und Vegetations-Eintrag (nur wenn Zone das Biom hat):
- **Seed**: `worldSeed + zoneID.x·4271 + zoneID.y·9187 + prefabHash` → deterministisch pro Zone+Prefab.
- **Anzahl**: `max < 1` → Chance; sonst `range(min, max+1)`. **Versuche**: `forcePlacement ? n·50 : n`.
- **Gruppen**: `groupCount = range(groupSizeMin, groupSizeMax+1)`, Folgepunkte im `groupRadius`.
- **Pro Instanz**: `rot_y = range(0,360)`, **`scale = range(scaleMin, scaleMax)`**, `rot_x/z = range(±randTilt)`.
- **Checks**: Biom-Bitmaske + BiomeArea, **Altitude relativ zu Wasser 30** (`minAltitude/maxAltitude`), Mistlands-VegMask, Ozeantiefe, Neigung (`cos(tilt)`), TerrainDelta (10 Stichproben), ForestFactor, ClearAreas + `radius`-Abstand zu bereits Platziertem.
- **Platzierung**: `snapToWater → y=30`; `y += groundOffset`; GroundTilt-Rotation; **Scale nur setzen wenn ≠ Prefab-Default**.
...
### Locations (`data/features.pkg` — 146 Locations)
- **Seed pro Feature**: `worldSeed + feature.hash`; Zonen-Ziehung uniform in [−range/64, range/64), verworfen ab 10000 m; range inkrementiert bei `centerFirst`.
...
## 2.6 Datenquellen im Referenzserver
...
| Hardcoded | Alle Weltgeometrie-/Biom-/Höhen-Parameter und Formeln im GeoManager | — |
...
2. **Exaktheit durch Tests**: `perlin_values.txt` als Golden-Master; zusätzlich Referenz-Höhen/Biome-Grids aus dem Referenzserver exportieren und gegen den TS-Port testen (gleicher Seed `KxSYuZquuw` → identische Welt).
...
| A3 | **Zufalls-Rotation + Scale vom Server**: Demo-Spawn mit zufälliger Y-Rotation und Scale (vorläufige Tabelle aus vegetation.pkg-Werten: FirTree 2,0–2,5, Pinetree_01 1,0–2,5, Beech1 0,8–1,5), als ZDO-Member `"scale"` speichern (Flag `SYNC_INITIAL_SCALE` nutzen) | `server/src/WovServer.ts` Z. 137–189 | S |
| A4 | **Client wendet ZDO-Scale + Rotation an** (statt Hints) | `client/src/engine/Renderer.ts` | S |

**Erwartetes Ergebnis**: Tannen 22–27 m hoch mit korrekter Silhouette (Cutout-Nadeln), Buchen 24–46 m — Wald sieht aus wie im Vorbild.
...
| # | Aufgabe | Aufwand |
|---|---|---|
| B1 | `util/Random.ts` — Unity-xorshift-RNG (Multiplikator `0x6c078965`, `next_float = (next_int() & 0x7FFFFF) · 1.192093e-7f`), inkl. exakter `range()`-Semantik | S |
| B2 | `util/Perlin.ts` — Unity-exakter Perlin: feste Permutationstabelle, Fade `6t⁵−15t⁴+10t³`, Normierung `(res+0.69)/1.483`. **Test gegen `data/tests/perlin_values.txt`** | S |
| B3 | `util/Mathf.ts` — LerpStep, SmoothStep, MathfLikeSmoothStep, Fbm, Remap, BlendOverlay | S |
| B4 | Seed-Hash prüfen: `getStableHash` ist bereits verifiziert — sicherstellen, dass derselbe djb2-Algorithmus für den Welt-Seed genutzt wird | XS |
| B5 ✅ | `util/FastNoise.ts` — Port (seed=0, Cellular/FBM) — nur für Ashlands; **erledigt (2026-07-24)**, s. Status-Update unten | M |

## Phase C — GeoManager portieren (P0) 🌍 ✅ **erledigt (2026-07-23)**

| # | Aufgabe | Aufwand |
|---|---|---|
| C1 | `shared/src/worldgen/GeoManager.ts`: Konstanten, Seed-Offsets (offset0–4, river/stream-Seed), WorldAngle | S |
| C2 | `GetBaseHeight` inkl. Kanal-Zerre, Weltrand, Berg-Deckel | M |
| C3 | Biom-Höhenformeln ×200: Meadows/Plains, BlackForest, Swamp, Mountain, DeepNorth, Mistlands, Ocean (Ashlands zunächst Legacy oder Stub) | M |
| C4 | `GetBiome` — Entscheidungslogik in exakter Reihenfolge + BiomeArea | M |
| C5 | Seen/Flüsse/Bäche-Generierung + AddRivers | L |
| C6 | **Test-Harness**: Referenzserver um Debug-Export erweitern (Grid von (x,z) → Höhe/Biom für Seed `KxSYuZquuw`) → TS-Test bis auf Float-Epsilon identisch | M |

## Phase D — Terrain: Server + Client (P0) 🏔️ ✅ **erledigt (2026-07-23)**

| # | Aufgabe | Betrifft | Aufwand |
|---|---|---|---|
| D1 ✅ | `shared/src/worldgen/Heightmap.ts`: 65×65-Grid pro Zone, Eckbiom-Blending mit SmoothStep auf tx/ty, vegMask | — | M |
| D2 ✅ | Höhen-Abfrage: nearest-vertex (bilinear=false ist aktiv) + Triangle-Raycast (Möller-Trumbore) für GroundData | — | M |
| D3 ✅ | **Client-Terrain durch echte Worldgen ersetzt**: `client/src/engine/Terrain.ts` — Chunk-Mesh pro Zone (65² Vertices, Dreiecksteilung der Referenz), distanzsortierte Bau-Queue (≤1 Chunk/Frame), Radius 4 Zonen, LRU über den geteilten HeightmapProvider | ersetzt `Renderer.ts` Z. 146–191 | L |
| D4 ✅ | **Wasser**: transparente Ebene bei y=30, folgt dem Spieler auf dem 64-m-Raster, dezente CPU-Sinus-Wellen (phasenstabil über Weltkoordinaten) | — | S |
...
| D6 ✅ | Server: GeoManager+HeightmapProvider in `init()` (~1,3 s), **ServerConfig-Paket** (Typ 52: worldName, seed, genVersion, Flags-Byte) bei Auth, Spawn y=Bodenhöhe, Gravitation (Fall 15 m/s, Boden-Snap) in `handlePlayerInput`, Demo-Welt auf echter Höhe; Client baut aus dem Paket den identischen GeoManager | `WovServer.ts` | M |
...
| # | Aufgabe | Aufwand |
|---|---|---|
| F1 ✅ | features.pkg → JSON: `tools/prefab-parser/parse-features.ts` → `shared/src/featuresData.json` (Feature-KÖPFE, Client+Server) **plus** `shared/src/featurePiecesData.json` (die Pieces, nur Server, hinter `shared/src/featurePieces.ts`; Bundle-Schnitt: die 8,5 MB Pieces hingen über den Barrel an jedem Client-Modul — der Kopf trägt stattdessen den vorberechneten `pieceRadius`, das Einzige, was `getTerrainLeveling` davon braucht) (**146 Features, 23 228 Pieces**, 12 054 mit RandomSpawn-Block — RandomSpawns per uint16-Index in Pieces gemergt, Semantik der Referenz); Registry `shared/src/features.ts` (`FEATURES` **in pkg-Reihenfolge** — Platzierungsreihenfolge ist determinismus-relevant) | M |
| F2 ✅ | Location-Platzierung in `server/src/world/ZoneManager.ts`: `prepareFeatures()` (PostGeoInit-Port, einmalig beim Start: pro Feature `State((seed+hash)\|0)`, GetRandomZone mit int-Division `(int)range/64`, 20 Punkt-Versuche, Checks in Reihenfolge der Referenz: Distanz → Biom (GeoManager, nicht Heightmap!) → Altitude (Override-aware) → Forest → **unbedingter** GetTerrainDelta (10 rng-Züge auch bei 0/0-Limits!) → Surrounding-Check → DistanceFromSimilar), `tryGenerateFeature` (ClearAreas: `clearArea`→exteriorRadius, sonst `terrain_modifiers.yml` level_radius; **randomRotation ist in der Referenz zeit-geseedet → absichtlich nicht welt-deterministisch**, gespiegelt), `generateFeature` (alle Pieces spawnen — chanceToSpawn wird in der Referenz nie ausgewertet; DUNGEON-Pieces gezählt+übersprungen bis Phase G; `pos + quat·piece.pos` f32; snapToTerrain-Override; LocationProxy-ZDO mit `location`/`seed`-Ints), `removeUngeneratedFeatures` (unique); PopulateZone-Reihenfolge Features→Foliage mit ClearAreas. Configs: `parse-yml-configs.ts` → `terrainModifiers.json` (35) / `locationOverrides.json` (ShipSetting01), `locationConfig.ts` (hash-keyed wie in der Referenz) | L |
...
- **Wind-/Vertex-Color-Shader** (COLOR_0 fehlt in GLBs — ggf. aus dem Extraktions-Export nachrüsten)
...
## Status-Update (2026-07-23): Phase B+C abgeschlossen und gegen die Referenz verifiziert ✅

**Phase B** (B1–B4): `shared/src/worldgen/Random.ts` (Unity-xorshift, Golden-Test bit-exakt), `Perlin.ts` (Unity-exakt, max. Abw. 2,3e-7), `Mathf.ts` (LerpStep/SmoothStep/Fbm/Remap/BlendOverlay), `getStableHash` gegen die Referenz verifiziert.

**Phase C** (C1–C6): `shared/src/worldgen/GeoManager.ts` — vollständiger 1:1-Port des Geo-Managers der Referenz inkl. Float32-Emulation (`Math.fround` pro Operation) und aller ihrer Eigenheiten (int-Offsets, Minimalwert-Init in GetTerrainDelta, f32-Literal-Vergleiche, Legacy-Ashlands-Branch in float32).

**C6-Verifikation** (Seed `KxSYuZquuw`, WorldGen v2, server.yml-Flags): Neues, rein additives Export-Tool für den Geo-Export im Referenz-Repo (**keine Änderung an Server-Sourcen**) schreibt `geo_structure.csv` / `geo_riverpoints.csv` / `geo_samples.csv`; TS-Harness `shared/test/geo-compare.ts` vergleicht bitgenau (ULP-Distanz auf f32-Bitmustern):
...
**⚠️ Nebenbefund — vermuteter Bug im Referenzserver (`GetBiomes`):** `Biome(to_underlying(GetBiome(...)) || ...)` nutzt logisches `||` statt bitweisem `|` → Rückgabe ist immer nur `None(0)` oder `Meadows(1)`, nie eine Bitmaske. Im Original-Unity-Code war es `|`. Wir haben den Fehler 1:1 portiert (mit Warnkommentar); die korrekte Variante liegt als `getBiomesMask()` daneben. Falls der Referenzserver `GetBiomes` irgendwo produktiv nutzt (z. B. Vegetations-Checks), entscheiden, ob dort gefixt wird — der Browser-Port folgt dann nach.

## Status-Update (2026-07-23): Phase D1/D2 abgeschlossen und gegen die Referenz verifiziert ✅

**D1/D2**: `shared/src/worldgen/Heightmap.ts` — 1:1-Port des Heightmap-Builders der Referenz (65×65 Vertices/Zone, Eckbiome, SmoothStep-tx/ty, Double-Lerp-Ketten, f32-Store) plus `HeightmapProvider` (LRU 512 Zonen) mit `GetWorldHeight` (nearest-vertex, bilinear=false) und Möller-Trumbore-Raycast.

Das Export-Tool schreibt jetzt zusätzlich `geo_zones.csv` (16 Zonen × [4 Eckbiome + 4225 Heights + 4096 vegMask] über den echten Heightmap-Builder der Referenz); `shared/test/heightmap-compare.ts` vergleicht:
...
| vegMask, 12 Single-Biom-Zonen | übersprungen — siehe Referenz-Bug #2 unten |
| Sanity `getGroundHeight(0,0)` | = Zonen-Vertex [32,32] ✓ |

**⚠️ Nebenbefund #2 — zweiter vermuteter Bug im Referenzserver:** Der Single-Biom-Fastpath deklariert die Variable `mask` **uninitialisiert** und speichert den Wert danach als `mistlandsMask`/vegMask. `GetBiomeHeight` schreibt die Maske aber nur für Mistlands/AshLands — für alle anderen Biome landet nicht-deterministischer Stack-Müll in die vegMask (Undefined Behavior). Der Multi-Biom-Pfad initialisiert korrekt `mask1 = 0, …, mask4 = 0`. → TS folgt dem initialisierten Verhalten (Mask=0); der Harness vergleicht vegMask nur auf Multi-Biom-Zonen. Praktisch folgenlos, solange die Maske nur für Mistlands-Features gelesen wird, aber im Referenzserver fixenswert (`mask` mit 0 initialisieren).
...
**E1–E5 umgesetzt.** Der Server bevölkert die Welt jetzt exakt wie der Referenzserver: `ZoneManager.ts` portiert `PopulateFoliage` 1:1 (alle 120 vegetation.pkg-Einträge in pkg-Reihenfolge, pro-Eintrag-RNG mit `(seed + zx·4271 + zy·9187 + prefabHash)|0`, rng-Ziehreihenfolge **vor** den Platzierungs-Checks, `rot_y` als int-Range `[0,360)`, TerrainDelta gegen das rohe `geo.getHeight` mit Minimalwert-Init der Referenz, Biom-Check über die Zonen-Ecken-Gewichtung der Heightmap — nicht das GeoManager-Biom). Spieler getriebene Zonen-Generierung mit Radius NEAR+DISTANT=4 (81 Zonen, Zentrum zuerst), `is_inside_world_radius` (164²), Queue mit 12-ms-Budget/Tick. Der Client rendert Vegetation als InstancedMesh pro Prefab (~1 Draw Call pro Art statt ~1 pro Objekt).
...
**Verifikations-Lücke (bewusst offen):** Die Platzierung ist deterministisch und algorithmisch 1:1, aber noch **nicht golden gegen den Referenzserver getestet** (anders als C6/D1). Für einen Bit-Vergleich müsste das Export-Tool die vom Referenzserver tatsächlich gespawnten Vegetations-ZDOs (Prefab, Position, Rotation, Scale) für ein Zonen-Set dumpen. Kandidat für eine spätere E-Verifikation.

## Status-Update (2026-07-24): Phase F komplett ✅ (F1–F3)

**Locations sind da.** Der Server bucht beim Start einmalig **11 397 Feature-Instanzen** (`PostGeoInit`/`PrepareFeatures` der Referenz: pro Feature ein eigener RNG-Strom `(seed+hash)|0`, Zone-via-int-Division, 20 Punkt-Versuche mit exakter Check-Reihenfolge der Referenz inkl. des **unbedingten** GetTerrainDelta-rng-Konsums pro Kandidat) und materialisiert sie zonenweise (`TryGenerateFeature` → Pieces + LocationProxy → ClearAreas → Vegetation). StartTemple steht in Zone (0,0) bei (6,9 / 37,0 / 6,9) — mit 23 Pieces (5 BossSteine + Deko), Runestones/WoodHouses/Eikthyrnir-Altäre in den umliegenden Zonen.

**Drei Erkenntnisse zur Referenz, die den Port prägen:** (a) Location-`randomRotation` nutzt einen **zeit-geseedeten** Default-RNG — die Rotation (und damit die Piece-Transforms rotierbarer Locations) ist in der Referenz absichtlich **nicht** welt-deterministisch; wir spiegeln das. (b) `randomSpawn.chanceToSpawn` wird in der Referenz zwar geparst, aber **nie ausgewertet** — alle Pieces spawnen immer. (c) `PrepareFeatures` läuft global **vor** jeder Zonen-Generierung; ClearAreas (features.pkg `clearArea`→exteriorRadius, sonst `terrain_modifiers.yml` level_radius) unterdrücken Vegetation in den Lichtungen.
...
**Verifikations-Lücke (wie bei E):** Platzierung deterministisch + algorithmisch 1:1, aber nicht golden gegen ZDO-Dumps der Referenz getestet. ~~AshLands-Locations nutzen Legacy-Höhen (B5-Lücke) und weichen dort vom Referenzserver ab~~ **seit B5 (2026-07-24) laufen die Altitude-/TerrainDelta-Checks in AshLands auf den exakten modernen Höhen der Referenz.**
...
**(a) Schwebende Location-Pieces (bis ~4,9 m an Hängen) — Ursache: fehlendes Terrain-Leveling.** Im Original ebnet die Geländeangleichung des Unity-Clients den Boden unter Locations auf ein Plateau (Zentrumshöhe + `levelOffset`); der Referenzserver implementiert das **nicht** (sein Terrain-Modifier liefert nur ClearArea-Parameter) und die Diagnose (`debug-float.ts`) wies exakt dieses Muster nach: Feature-Zentren perfekt (±0,1 m), Pieces bis 4,89 m über Grund. Neu: `Heightmap.heights` = `baseHeights` + beim Zonen-Build **gebackenes** Leveling (Unity-Formel: Plateau im `levelRadius`, Blend-Band `t=((dist−r)/smoothRadius)^smoothPower`, `square`-Norm als Chebyshev-Distanz), mit identischer shared-Mathe auf Server (Ground Truth: Spieler-Physik, Vegetations-Raycast) und Client (Chunk-Meshes). `baseHeights` bleibt pristine (D1-Goldentests, oceanDepth, Parität mit der Referenz). Parameter-Regel (`getTerrainLeveling`): `terrain_modifiers.yml` (35 Features) → yml-Werte; sonst `clearArea` ⇒ levelRadius=exteriorRadius + Defaults der Referenz (−0,2 / 7 / 3 / square). Registrierung beim Generieren (`tryGenerateFeature`, Zielhöhe = gebuchte Geo-Höhe + levelOffset); der Client leitet denselben Modifier aus dem LocationProxy-ZDO (`location`-Int-Member) ab und baut betroffene Chunks neu. **Tempel-Messung: alle 23 Pieces exakt auf pkg-Offset über dem Plateau (Invarianten-Verletzung 0,0000 m; max |piece−ground| = 0,200 m = der designede Einbett-Offset, vorher 2,40 m Hang-Fehler).**
...
**Der letzte Worldgen-Blindflug ist geschlossen.** `shared/src/worldgen/FastNoise.ts` ist ein 1:1-Port des **originalen** FastNoise aus dem Referenz-Client (kein FastNoiseLite): 2D-Simplex (F2/G2, ×50) und Cellular (Euclidean-Distance, **quadriert** — ohne sqrt, jitter `f32(0,45)`), FBM mit `++seed` pro Oktave und lacunarity 2 / gain 0,5, der Hash der Referenz mit int32-Überlauf über `Math.imul`, `fastFloor` ≠ `Math.floor` (abweichend bei negativen Ganzzahlen). Die 256 Einträge der CELL_2D-Tabelle wurden maschinell gegen den Header der Referenz diffiert (**identisch**). Konstruktion exakt wie in der Referenz: seed=0 (nicht Welt-Seed), `setFrequency(f32(0,01))`, 2 Oktaven. (3D-Tabellen weggelassen — keine Aufrufer.)

`GeoManager.getAshlandsHeightModern` portiert den modernen Ashlands-Höhenpfad Zeile für Zeile mit f32/f64-Disziplin an jeder Cast-Stelle der Referenz: value-Rim-Kurve (12 000-m-Rand, magnitudeF-Args als f32), 10 150-m-Band, Shift `+= f32(100000 + offset3)`, zwei Cellular-Oktavenschleifen (5× ab 0,33·2ⁱ, 3× ab 8·2ʲ), blendOverlay/mathfLerp/smoothStep-Kette, Simplex-Fraktal-Multiplikator, fbm-Plateau, die **Lava-Maske** `blendOverlay(num18, num14) · clamp01((num12−0,17)/0,01)` (fließt über den bestehenden vegMask-Kanal wie die Mistlands-Maske) und den Boden-Absenker nahe der Baseline. `getBiomeHeight` nutzt im modernen Modus diesen Pfad (×200 als f32); die **Fluss-Generierung** (`preGeneration=true`) bleibt wie in der Referenz immer Legacy — als identisch in beiden Modi verifiziert.

**Flag mit Parität zur Referenz an:** `experimental-ashlands-modern-noise: true` ist Default der Referenz (Referenzserver + dessen server.yml) — jetzt auch bei uns (DEFAULT_CONFIG true, main.ts `?? true`, server.yml-Schlüssel ergänzt); der bisherige Start-Downgrade auf Legacy ist entfernt.
...
**Nebenbefund Original-Abgleich (Original-Client):** die Original-Biom-Verdrahtung bestätigt unsere Splat-Architektur — die Vertex-`Color32` ist eine **Biom-Maske**, keine Farbe (Swamp=R, Mountain=G, BlackForest=B, Plains=A, AshLands=R+A, DeepNorth=G, Mistlands=B+A, Meadows/Ocean/None=(0,0,0,0)), geblendet via `Color32.Lerp` mit Smoothstep-Gewichten über die Zonen-Eckbiome — strukturell identisch zu unseren cornerBiomes×smoothstep-Gewichten in aTiles/aWeights. Die Biom-Index-Tabelle dient im Original nur der Dominant-Biom-Abfrage, nicht den Tiles.
...
**Kontext/Entscheid:** Die Referenz enthält **kein** server-seitiges Spawn-System — im Original spawnt der besitzende Unity-Client (Regeln in den Spawn-Listen des Zonensystems, in keinem der beiden Repos vorhanden), der Referenzserver repliziert nur die resultierenden ZDOs. Da unsere Browser-Architektur keinen privilegierten Client hat, läuft das Spawnen bei uns server-seitig — mit einer **autorierten** (nicht portierten) Tabelle, weil keine Referenzdaten existieren (Einschränkung #26).
...
**Fix:** `sendTimeSync(peer)`-Helper in `WovServer.ts` (ersetzt die drei duplizierten Paket-Bausteine Connect/setTimeOfDay/neu) + **1-Hz-Broadcast am Ende von `update()`** (~20 B/s/Peer — vernachlässigbar; HUD-Auflösung ist ohnehin minütlich, Beleuchtungs-Schritte von 1 s sind unmerklich). `tsc --noEmit` sauber.
...
**Befund (via Daylight-Proben):** in den Screenshots blieben ~10 % der StaticInstancer-Buckets dauerhaft als beschriftete Placeholder-Boxen stehen (allein um den Spawn-Meadows ~46 Boxen). Die Bucket-Diagnose (`tools/pw-deer-diag.mjs`) zeigte: **kein einziges Welt-Modell fehlt** — die roten „stonerock"-/„FirTree_oldLog"-Boxen der frühen Proben waren transiente Ladezustände der frischen SwiftShader-Session (heilen spätestens über den 35-s-Retry). Die sechs *wirklich* permanenten Placeholder sind ausschließlich **Logic-/Marker-Prefabs**, die auch im Original unsichtbar sind: `Music_MeadowsVillageFarm` (13×), `Spawner_Boar` (18×), `Spawner_Skeleton_night_noarcher` (4×), `BlackForestLocationMusic` (4×), `Flies` (1×) — plus `Pickable_DolmenTreasure` (6×, s. Einschränkung #31). Ursachen: vier haben **keine GLB im Export** (404 → Placeholder), zwei (`BlackForestLocationMusic`, `Flies`) haben **0-Mesh-GLBs** (leere Hierarchie-Exporte — Klasse wie Einschränkung #27, nur ohne Knochen).
...
3. **Letzte permanente Placeholder-Box:** `Pickable_DolmenTreasure` (Einschränkung #31) — 30 s nach Connect + Bucket-Scan bestätigte: einziger verbleibender Bucket mit `sourceMeshes: 0` (6 Instanzen). GLB-Inspektion (`tools/glb-inspect.js`) bestätigte einen 0-Mesh-Export (kein Material/Mesh, nur ein leerer Root-Node) — dieselbe Klasse wie die bereits gefilterten Logic-Prefabs. Da der Nutzer nun explizit **alle** nicht ladenden Prefabs gefixt haben wollte (nicht mehr „bewusst akzeptiert" wie zuvor entschieden), wurde `isRenderable()` in `shared/src/prefabs.ts` um `Pickable_DolmenTreasure` erweitert — konsistent mit der bestehenden Logic-Prefab-Filterung, und tatsächlich korrekter als vorher: im Original ist dieser Pickup-Trigger ebenfalls unsichtbar (das sichtbare Loot ist die separate `treasure_pile`-Dekoration).
...
**Auftrag des Nutzers:** die eigene Engine soll sich „so nah wie möglich wie die Unity Engine" verhalten, „damit Texturen usw. dann auch richtig ausstehen". Ausgangspunkt war ein Audit des Terrain-Texturier-Pipelines gegen den Referenzserver und den Unity-Client des Originals.

**(1) Ground-Truth-Fund.** Der kompilierte Terrain-Shader-Blob des Exports ist nicht praktikabel zu HLSL zurückzuübersetzen — dieser Weg wurde verworfen. Der Extraktions-Export enthält aber in `tools/asset-export/` zusätzlich zum gestapelten `terrain_d_array.png` auch **einzeln benannte Slices** (`terraintile_0.png`…`terraintile_15.png`), die exakte, unabhängige Kopien der 16 Array-Layer sind. Sichtprüfung aller 16 Dateien gegen die bestehende `TILE`/`BIOME_TILE`-Zuordnung in `Terrain.ts`: **16/16 Treffer, 0 Abweichungen** — die zuvor als „hand-rekonstruiert, ggf. vertauscht" dokumentierte Zuordnung war die ganze Zeit korrekt und ist jetzt als verifiziert dokumentiert (Einschränkung 5).
...
**(4) Datenintegrität gefunden und repariert.** Die im Texturordner des Prototyps (die vom Dev-Server unter `/assets/` ausgelieferte Ordnerkopie) liegenden `terraintile_*.png`-Dateien waren **0 Byte** (stub/leer) — 2672 von 2749 Dateien in diesem Ordner sind das, nur ein Teil der Assets wurde tatsächlich vollständig exportiert. Die echten Daten lagen nur im Extraktions-Ordner unter `tools/` (17–193 KB je Datei). Alle 19 benötigten Dateien (16 Diffuse-Slices + 3 Normal-Tiles) wurden dorthin kopiert, damit `loader.loadAsync('/assets/textures/terraintile_n_*.png')` echten Inhalt statt eines Decode-Fehlers liefert.
...
**(3) Datenintegrität — gleiche Bug-Klasse wie bei G-TEX2.** Die benötigten Texturen im Texturordner des Prototyps waren wieder 0-Byte-Stubs (`grass_meadows.png`, `grass_heath.png`, `water_normals.png`); echte Kopien lagen im Extraktions-Ordner unter `tools/`. Nur diese 3 konkret benötigten Dateien wurden kopiert (bewusst nicht das gesamte Textur-Verzeichnis — 2672+ leere Dateien laut G-TEX2-Messung sind ein separates, viel größeres Aufräum-Thema).
...
**Verifikation:** Probe-Bericht: `[GrassClutter] loaded`, 19 Chunks/16.587 Instanzen, Wasser Normal-Map 0,7 auf beiden Ebenen. Screenshots `gw-ground.png`/`gw-water.png): vollflächige grüne Wiese ohne schwarze Flecken, Wasser mit klar lesbarer Wellenstruktur. `npx tsc --noEmit -p client` sauber.

## Status-Update (2026-07-26): Bäume zu groß + Texturen falsch — Root-Causes aus Code-Analyse ✅

**Kontext:** Dieses Dokument galt bisher für den three.js-Prototyp. Der Babylon-Client (Babylon.js 8, WebGPU/WebGL, Thin-Instances statt InstancedMesh) zeigte nach Phase-2-„Fertigmeldung" zwei Nutzer-Beanstandungen: **(a) Bäume zu groß / Proportionen falsch**, **(b) Texturen passen nicht** (Bäume als solide Blöcke, Rinde mit Lochfraß, Blätter opak). Beide wurden **von der Code-Seite** analysiert (Babylon-Quelltext in `node_modules` + GLB-Binärparse) statt nur per Screenshot-Gerate.

### Befund 1 — „Zu große Bäume": ZWEI verschiedene Dinge liefen falsch

1. **Erwartung vs. Datenlage.** Die GLB-Naturgrößen sind korrekt (Messung via neues Tool `tools/glb-size-check.mjs`, parst glTF-JSON+BIN direkt, wendet die Node-Hierarchie-Matrizen an): **Beech1 30,5 m**, Birch1 19,5 m, Oak1 24,8 m, FirTree 12,7 m (inkl. `Pine_tree`-Node-Scale ×2), Pinetree_01 25,1 m. Vegetations-Skalen aus vegetation.pkg: Beech1 **0,8–1,5** → Weltbuche **24–46 m**. Das entspricht dem Original und ist **kein** Bug — im three.js-Projekt stand derselbe Wald; ein subjektiv „zu großer" Eindruck entsteht nur, weil der Babylon-Client aktuell ohne dichten Nebel rendert und die Kamera-FOV eng ist. Der Analyse-Befund aus §1.3/§2.4 gilt unverändert: **Skala = GLB-Naturgröße × scaleScalar-Zufall**, keine Höhen-Normalisierung — der Babylon-Client hatte diese Falle von Anfang an nicht (gut).
...
**Fix (`client/src/engine/AssetManager.ts`):** statt `forceAlphaTest` jetzt `material.transparencyMode = Material.MATERIAL_ALPHATEST` + `alphaCutOff = 0.5` + `useAlphaFromAlbedoTexture = true` + `tex.hasAlpha = true` (der Export schreibt RGBA mit `hasAlpha=false`, obwohl echte Alpha-Daten vorhanden sind). Laufzeit-Verifikation: Shader-Defines zeigen jetzt `#define ALPHATEST`, `#define ALPHAFROMALBEDO`, `#define ALPHATESTVALUE 0.5` — der Discard ist damit **garantiert** aktiv (nicht nur „sieht besser aus").

**Bark-Bug (Lochfraß in Stämmen/Felsen) — Messung statt Schwelle:** die exportierten Texturen führen im Alpha-Kanal nicht nur Transparenz: `beech_bark` 7,6 % / `birch_bark` 45 % transluzente Pixel (Smoothness/Masken-Kanal), echte Laub-Cards `beech_leaf` 85 % / `birch_leaf` 82 %. Fix: Cutout nur bei **> 50 %** transluzenten Pixeln — Rinde bleibt opak, Laub bekommt den Cutout (vorher griff jede Textur mit *irgendeinem* Alpha-Pixel, inkl. `needDepthPrePass`-Nebeneffekt „invertierte Helligkeit", der mitentfernt wurde).
...
- **Exportierte Texturen:** `hasAlpha=false` trotz echtem Alpha-Kanal; Alpha ist bei Rinde/Felsen ein Smoothness-/Masken-Kanal (Messung statt Fixpunkt-Schwelle).
...
| 2 | **Keine Vertex-Farben (COLOR_0) in den GLBs** → kein Wind-/Jahreszeiten-Shader wie im Original möglich | Ggf. aus dem Extraktions-Export nachrüsten (Phase G) |
...
| 8 | **vegMask des Referenzservers ist auf Single-Biom-Zonen UB-Müll** (uninitialisierte Variable `mask`, s. Status-Update D1). TS liefert dort das beabsichtigte Verhalten (0), weicht also bewusst vom Ist der Referenz ab. Praktisch folgenlos: Die Maske wird nur gelesen, wenn `minVegetation != maxVegetation` (Mistlands-Einträge), und Mistlands-Zonen sind nie Single-Biom-Fastpath-Fälle mit unkritischer Maske | Fix im Referenzserver (`mask` mit 0 initialisieren) — danach Export neu fahren und Harness-Skip entfernen |
| 9 | **Zonen-Generierung läuft auf dem Server-Hauptthread** (~30 ms/Zone, 12-ms-Budget/Tick ⇒ ~1 Zone/Tick). **G-POP: entschärft** — die Queue wird jetzt gecullt (Zonen > 5 Zonen Chebyshev von allen Peers fliegen raus) und nearest-first sortiert: beim Flug entsteht die Landschaft **vor** dem Spieler zuerst, das „Aufholen hinter einem" ist weg. Übrig bleibt die rohe Generierungsrate (~3 s für die 81 Zonen beim Login; Bäume poppen nacheinander rein — wie im Original beim ersten Betreten, nur langsamer). Die Referenz lagert Heightmaps in einen Thread-Pool aus | Worker-Thread für Heightmap+Populate (Phase G) |
...
| 11 | **ZDO-Rotation wird im TS-Protokoll als Quaternion übertragen**, der Referenzserver speichert/synct Euler-Winkel. Visuell gleichwertig; relevant erst bei Save-File-Binärkompatibilität | Phase G (Persistenz-Format an die Referenz anlehnen) |
| 12 | **`prepareFeatures()` braucht ~75 s beim Server-Start** (einmalig, Hauptthread): die ~2–4 Mio. verworfenen Kandidatenpunkte (Biom-/Altitude-Checks über Noise) kosten in JS spürbar mehr als nativ (wenige Sekunden). Determinismus bleibt unberührt | Biom-Prefilter oder Worker für die Platzierung (Phase G), ggf. getBiome-Cache |
| 13 | **Location-`randomRotation` ist absichtlich nicht welt-deterministisch** — Die Referenz nutzt dafür einen zeit-geseedeten Default-RNG; Rotation + Piece-Transforms rotierbarer Locations unterscheiden sich pro Server-Start (auch in der Referenz!). Wir spiegeln das Verhalten statt es „zu fixen" | Bewusst akzeptiert (Parität mit der Referenz); für Golden-Tests nur Positionen/Prefab-Multimengen vergleichen |
| 14 | **DUNGEON-Pieces werden übersprungen** (dungeons.enabled=true ⇒ die Referenz routet sie an den DungeonManager) — Dungeon-Locations (SunkenCrypt, MountainCave, …) haben noch keinerlei sichtbare Struktur; Zähler wird pro Location geloggt | Phase G (Dungeon-Generierung) |
| 15 | **Terrain-Leveling (F4) weicht bewusst von der Heightmap der Referenz ab** — der Referenzserver ebnet den Boden unter Locations nicht (sein Terrain-Modifier liefert nur ClearArea-Parameter); im Original ist das Unity-Client-Verhalten. Unsere Höhen-Abfragen (`getGroundHeight*`, Normalen) liefern nahe Locations das geebnete Plateau — Server und Client bleiben über die identische shared-Mathe konsistent, weichen aber dort vom Referenzserver ab. Parameter: `terrain_modifiers.yml` (35 Features), sonst clearArea⇒exteriorRadius + Defaults der Referenz; die echten Unity-Prefab-Einzelparameter sind unbekannt (Annäherung) | Bewusst akzeptiert (Unity-Parität); bei künftigen Golden-Dumps Höhen in Location-Nähe gesondert klassifizieren |
...
| 20 | **Save-Format ist ein eigenes JSON+zstd-Envelope** (`<world>.db.zst` via `node:zlib`), nicht binärkompatibel zum `.db`/`.fwl` der Referenz (DataWriter-Layout). Die Inhalte spiegeln die Referenz (meta ≙ .fwl, worldTime, Zonen, persistente ZDOs, Spieler), aber Saves sind nicht zwischen Referenz- und TS-Server austauschbar; RandomEventManager/GlobalKeys fehlen beidseitig noch | Falls Save-Austausch je gewünscht: DataWriter-Layout der Referenz nachziehen; GlobalKeys mit RandomEvents (Phase G) |
| 21 | **Charakter-ZDOs von Spielern werden nicht als ZDOs gespeichert** (Ghost-Schutz, s. G1-Update) — Position/Fly-Status leben in der `players[]`-Sektion und werden beim Connect **per Name** zugeordnet. Folge: Name = Identität; ein Namenswechsel bedeutet ein „neuer" Charakter (Spawn am Starttempel). Die Referenz speichert Charakter-ZDOs und gleicht sie über die Session ab | Langfristig: stabile Charakter-ID beim ersten Connect vergeben (vorerst bewusst einfach) |
| 22 | **`randomRotation` ist seit G1 über Restarts stabil** (Verbesserung, keine Lücke): erzeugte Location-Pieces werden als ZDOs persistiert — eine geladene Location rotiert nicht erneut. Einschränkung 13 gilt damit nur noch für die **erste** Generierung einer Zone (frische Welt / neu betretene Zonen) | — (Parität mit der Referenz erreicht) |
...
| 24 | **Nebel ist seit G-POP deutlich dichter** (0,0028/0,0032/0,0042 Tag/Dämmerung/Nacht statt pauschal 0,0015) — bewusste Stil-Entscheidung: die typisch verhangene Fernsicht des Vorbilds, die Ring-Rand (640 m) und verbleibende Pop-in-Reste kaschiert; im Original wäre bei klarem Wetter weiter sichtbar | Optional: Nebeldichte an Wetter/Tageszeit koppeln (Wetter-System mit RandomEvents, Phase G) |
...
| 26 | **Kreaturen-Spawn-Regeln sind autoriert, nicht portiert** — die Referenz hat kein server-seitiges Spawn-System (im Original spawnt der besitzende Unity-Client; dessen Spawn-Listen-Daten liegen in keinem Repo). Tabelle/Intervalle/Chancen/Radien/Geschwindigkeiten in `shared/src/spawnData.ts` sind nach Vanilla-Gefühl gesetzt, nicht 1:1 | Bei Bedarf Werte aus dem Unity-Client des Originals extrahieren und angleichen |
...
| 28 | **Kreaturen haben keine Animationen** — weder Skins noch AnimClips im GLB-Bestand, kein AnimationMixer im Client; sie gleiten in Bind-/Idle-Pose über den Boden (Bewegung selbst ist weich interpoliert, G2). **Nachgemessen 2026-07-27:** die Lücke ist total — **keine einzige der 7.471 GLB-Dateien** in `assets/models/` hat einen `animations`- oder `skins`-Eintrag (direkt aus dem glTF-JSON-Chunk geprüft) | Animationen exportieren (Extraktion) + AnimationMixer im Renderer (Phase G+) |
...
| Ashlands | jetzt mit FastNoise-Port vs. später | ~~später~~ **erledigt (B5, 2026-07-24)** — moderner Pfad ist Default (Parität mit der Referenz) und golden verifiziert |
