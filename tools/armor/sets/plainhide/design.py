"""The Plainhide design: what every new character wears.

A rough hide shirt with a wide stitched neck, a plain band as a belt, a short skirt of
the shirt over hide trousers, linen under the sleeves, hide wraps on forearms and shins,
laced turnshoes. No metal, no emblem, no glow.

Five items, not seven: head and hands stay the character's own body (free_regions in the
configuration), so their skin tone, face, hair and beard are never replaced. No replaced
region shows skin: upper arms, forearms and shins are clothed, not bare. The scaffold's
lining is the garment itself: shirt and trousers are softened (smoothed and let out a few
millimetres inside their borders) so the cloth does not show the body's facets.
Everything added is hems, seams, band and wraps laid onto each body's own lining by ray
casts, so both bodies are fitted. The preview glow is off (preview_glare 0).

design(ctx) adds the geometry to ctx.pieces[<region>] and the triangle budget to
ctx.budget. after_export(ctx) adds the review images the scaffold does not make (a true
side view; front and back in a quick run) and the witnesses: five items, head and hands
free, nothing glows, five materials at most, no head or hand mesh node in any GLB. The
geometry is the one the set shipped with.
"""
import json
import math
import struct
...
def design(ctx):
    rig, pieces, materials, PARTS = ctx.rig, ctx.pieces, ctx.materials, ctx.PARTS
    mesh, binding_surface, attach_to_surface = ctx.mesh, ctx.binding_surface, ctx.attach_to_surface
...
def after_export(ctx):
    ROOT, QUICK, PARTS, SLOTS, PREFIX, FREE = ctx.ROOT, ctx.QUICK, ctx.PARTS, ctx.SLOTS, ctx.PREFIX, ctx.free_regions
    armor, report, budget, equipment = ctx.armor, ctx.report, ctx.budget, ctx.equipment
    cam, camera, render, target = ctx.cam, ctx.camera, ctx.render, ctx.target
