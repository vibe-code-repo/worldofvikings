...
const [dungeonId, xArg, zArg, yawArg = '0'] = process.argv.slice(2);
if (!dungeonId) {
  console.error('Aufruf: node tools/elements/pruefung/relief-kontrast.mjs <dungeonId> [x] [z] [yawGrad]');
  process.exit(2);
}
 ...
 */
async function messung(relief, marke) {
  ...
  await seite.goto(`${HOST}/?name=${name}&dungeon=${dungeonId}&relief=${relief}`, {
    ...
    for (const m of window.__dbg.scene.meshes) {
      for (const sm of m.subMeshes ?? []) {
        ...
        for (const z of d.split('\n')) {
          if (/^#define STEIN_/.test(z.trim())) gefunden.add(z.trim());
        }
      }
    }
  ...
  const pfad = `${ORDNER}/${dungeonId}-relief-${marke}.png`;
...
for (const [marke, m] of [['aus', aus], ['an', an]]) {
  console.log(`relief=${marke}  Bild ${m.pfad}`);
...
if (!an.defines.includes('#define STEIN_NORMAL_WAND')) gruende.push('kein STEIN_NORMAL_WAND im übersetzten Shader — fehlt <albedo>_normal.png?');
if (aus.defines.includes('#define STEIN_NORMAL')) gruende.push('relief=0 hat den Kanal trotzdem an — der Schalter wirkt nicht');
