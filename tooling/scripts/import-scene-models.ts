...
/**
 * Every texture the store already holds, by content hash.
 *
 ...
 */
const textureByHash = new Map<string, string>();
try {
  for (const fileName of (await readdir(join(storeRoot, 'textures'))).sort()) {
    if (!fileName.endsWith('.png')) {
      continue;
    }
    const bytes = await readFile(join(storeRoot, 'textures', fileName));
    textureByHash.set(hashOf(bytes), `textures/${fileName}`);
  }
} catch {
  process.stdout.write('  store has no textures/ folder yet\n');
}
...
for (const group of plan) {
  ...
  for (const image of cut.images) {
    ...
    let texturePath = textureByHash.get(hash);
    if (texturePath === undefined) {
      ...
      texturePath = `textures/${group.stem}-${hash.slice('sha256-'.length, 'sha256-'.length + 8)}.png`;
      textureByHash.set(hash, texturePath);
    ...
    if (target !== undefined) {
      // Relative to the GLB's own URL, the convention the store already uses.
      target.uri = `../${texturePath}`;
    }
  }
