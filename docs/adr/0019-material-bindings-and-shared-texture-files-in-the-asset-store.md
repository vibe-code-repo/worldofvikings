...
4. Each bound material's base colour becomes a **file** in the model group's own
   `textures/` folder, named `<material-kebab>-<hash8>.png`, deduplicated by
   content hash, and halved until it fits the 2048 px budget.
...
6. Which materials are cut out, double-sided or self-lit comes from a **checked
   table by material name** (`materials.ts`), listing all 58 names in the
   export, not from a pattern over names.
...
a person can review by material name.
