...
import { Inventory, IRONWARD_PARTS, findItem, decodeArmor } from '@wov/shared';
...
appearance({ kopf: 'ironward_helm' }); assert.equal(peer.ruestung, '|', 'Cannot equip unowned armor');
for (const p of IRONWARD_PARTS) peer.inventar.addItem(findItem(p.item)!, 1);
...
appearance({ kopf: 'ironward_brust' }); assert.equal(peer.ruestung, accepted, 'Reject wrong slot');
peer.inventar.removeByName('IronwardHelmet', 1); server.inventarSync(peer);
...
let result = server.adminCommands.execute(admin, 'item ironward Gast'); assert(result.ok, result.message);
assert.equal(saved.inventar.length, 8); assert.deepEqual(saved.position, { x: 4, y: 5, z: 6 });
result = server.adminCommands.execute(admin, 'item ironward Gast'); assert(result.ok);
assert.match(result.message, /0 neue/); assert.equal(saved.inventar.length, 8); assert.equal(saves, 2);
assert(!server.adminCommands.execute({ isAdmin: false } as any, 'item ironward Gast').ok);
assert(!server.adminCommands.execute(admin, 'item ironward Nobody').ok);
...
assert(!server.adminCommands.execute(admin, 'item ironward Gast').ok);
...
console.log('PASS Ironward server: ownership, slots, equipped flags, legacy packets, atomic and idempotent delivery, admin gate');
