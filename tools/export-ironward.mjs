  ...
  for (const sourceRegion of part.sourceRegions ?? part.regions) {
    ...
    for (const primitive of source.json.meshes[node.mesh].primitives) {
      ...
      if (!materialMap.has(primitive.material)) {
        ...
        // Preserve declarations for material extensions such as emissive strength.
        for (const extension of Object.keys(mat.extensions ?? {})) {
          doc.extensionsUsed ??= [];
          if (!doc.extensionsUsed.includes(extension)) doc.extensionsUsed.push(extension);
          if (source.json.extensionsRequired?.includes(extension)) {
            doc.extensionsRequired ??= [];
            if (!doc.extensionsRequired.includes(extension)) doc.extensionsRequired.push(extension);
          }
        }
      }
