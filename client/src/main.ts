...
import type { NpcEinordnung, SteinKitConfig, TerrainComp } from '@wov/shared';
...
async function main() {
  ...
  const aussehenTeile = (): Record<string, string | null> => {
    const teile: Record<string, string | null> = {
      // Nur H_01 ist der nicht zum neuen Wikingerkopf passende Alt-Export.
      frisur: selectedFigure === 'wikinger' && frisurZu(selectedHairstyle).id === 'H_01'
        ? null
        : frisurZu(selectedHairstyle).datei,
  ...
  const uebernehmeAussehen = (): void => {
    ...
    socket?.sendAussehen(
      k.frisur, k.ober, k.beine, k.haarfarbe, k.augenfarbe, equipment?.aussehen()
    );
  ...
  // ?layout=editor loads the editor draft on top — the "Testflug": walk the
  // unpublished world on the real game terrain. The whole editor part lives in
  // editor/testflug/ and reaches this closure only through the context below.
  if (offlineMode) {
    const persistenz = localStoragePersistenz();
    const testflug = ladeTestflugEntwurf(params, persistenz);
    ...
    starteTestflug(
      {
        ...
        terrain: () => terrain,
        grass: () => grass,
        sockelFreiflaechen: () => sockelFreiflaechen,
        setzeSockelFreiflaechen: (liste) => {
          sockelFreiflaechen = liste;
        },
        setzeSpawnEditorOffen: (istOffen) => {
          spawnEditorOffen = istOffen;
        },
