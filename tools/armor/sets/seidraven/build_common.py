...
Shared implementation. Run it through the per-body entry points, which set the
variant: male/build.py (source WoV_BodyBase_Male.blend) or female/build.py
(source WoV_BodyBase_Female.blend, appends --female).
Also the scaffold of ../emberrage/build_common.py, which executes the text of this
file up to the design marker below and after the join marker further down.

Run Blender with --factory-startup -b SOURCE.blend --python THIS.py -- OUTPUT
and optionally --quick (hero only) and --female. Geometry is deterministic, no
paid APIs.
"""
import bpy
import bmesh
