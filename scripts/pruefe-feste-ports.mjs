 ...
 * (247n game server, 248n admin service, 529n client for slots 0-8; 2709-2713, 2829-2833,
 * 5842-5846 for slots 9-13; see AGENTS.md 3) AND starts a
 ...
 * 5299 is fixed in tools/dungeon2-*): 2700+n, 2820+n, 5833+n = 2709-2713, 2829-2833, 5842-5846.
 * Not part of a decimal number (`0.2489`, `-5292.9`).
 */
const SLOTPORT = /(?<![\d.])(?:247\d|248\d|529\d|270[9]|271[0-3]|282[9]|283[0-3]|584[2-6])(?![\d.])/;
  ...
  for (const datei of skriptdateien(wurzel)) {
    ...
    if (!datei.istTest) {
      ...
      if (!namen.has(datei.rel) && code.some((z) => SLOTPORT.test(z)) && code.some((z) => STARTET_SERVER.test(z))) {
        funde.push(`${datei.rel}: starts a server on a slot port (247n/248n/529n, slots 9-13: 2709-2713/2829-2833/5842-5846) and is not on WERKZEUGE - take a free port or name it there with a reason`);
      }
      continue;
    }
...
function selbstprobe() {
  ...
  try {
    ...
    schreibe('tools/neuer-slot.mjs', "const CLIENT_PORT = 5844;\nspawn('vite', ['--port', String(CLIENT_PORT)]);\n");
    schreibe('tools/neuer-slot2.mjs', "createWovServer({ port: 2711 });\n");
    schreibe('tools/nachbar.mjs', "const a = 5841, b = 5847, c = 2708, d = 2714, e = 2828, f = 2834; spawn('vite', ['--port', String(a)]);\n");
