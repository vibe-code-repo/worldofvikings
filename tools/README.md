...
1. `tools/store-vegetation-aufbereiten.mjs` schreibt `assets/store-lab/vegetation/` und `shared/src/laubVorbilder.ts` (**mit `tsx`** seit 12.09.2026 — es liest die Farben des Vorbilds aus `shared/src/laubSpitzen.ts`),
...
| `store-terrain-schichten.mjs` | Baut die Bodenschichten des Vorbilds als Texturstapel für den Splat (Stufe 2, „Look"). Liest die sechs handgemalten Terrain-Texturen aus `assets/store/textures/` (Farbe **und** Normalmap je Schicht) und schreibt `assets/generiert/terrain/store_d_array.png`, `store_n_array.png` und `assets/generiert/terrain/store-schichten.json` — 16 Zeilen à K², **dieselbe Form wie der Altbestand-Stapel `terrain_d_array.png`**, damit der Shader seine Zeilennummer weiter aus dem Tile-Index des Vertex rechnet und Farbe und Normale nicht auseinanderlaufen können. `--kante 256|512|1024|2048` (Vorgabe **512**, gemessen — s. unten), `--nur-pruefen` druckt nur die Tabelle. Zwei Zeilen bleiben Altbestand: **Asche** (im Store ohne Entsprechung) und **Lava-Kruste** (die ist gar keine Farbkachel, sondern die Graustufen-Emissionsmaske der glühenden Risse — eine Store-Textur dort legte die Risse dorthin, wo ihr Gestein hell ist). Die vier Zahlen je Schicht — Kachelmaß, Normalstärke, Metallic, Glätte — sind nicht geraten, sondern der `terrain`-Block der Zone „village" aus der Weltdatei village1.json des Vergleichsprojekts (ADR-0032); sie stehen zwangsläufig ein zweites Mal in `SCHICHT_OBERFLAECHE` von `client/src/engine/TerrainSplat.ts`, weil der Shader beim Bauen des Materials entsteht und `assets/generiert/` erst zur Laufzeit da ist. Deterministisch und idempotent: zweiter Lauf byteidentisch (fester `lanczos3`-Kern, feste PNG-Einstellungen, kein Zeitstempel). Wächter: `tools/test/terrain-schichten.ts` — Determinismus, Gleichheit der beiden Zahlenlisten, Vollständigkeit der Steigungsrampe je Biom (alle 16 Zeilen tragen Hang-, Fels- und Rau-Kachel), die drei Schwellen der Rampe (`RAMPEN`: monoton, berühren sich statt sich zu überschneiden, oberste Stufe unter 56° und damit auf unseren Inseln überhaupt erreichbar), dass der Fels der grasigen Biome metallfrei ist (Tile 4 `terrain-rock-a` ist im Original „Ani Dark Rockwall“ mit Metallic 0,85 auf Albedo sRGB 23–49 und wird ohne HDR-Umgebung schwarz — der helle Fels ist Tile 5), die sechs Zahlen der Himmels-Irradianz (`HIMMEL_IRRADIANZ`, gegen eine stumpfe Summe über 20 000 Richtungen) und dass beide Schalter (`STORE_BODEN_AKTIV`, `BODEN_FACETTIERT`) als `boolean` typisiert bleiben. Im Sammellauf hängt er hinter `brauchtBodenQuellen()` und nicht hinter `brauchtStore()`: Er braucht BEIDE Ordner, und mit Speicher aber ohne Altbestand brach er vorher hart ab, statt übersprungen zu werden. **Feinabgleich 09.09.2026:** Drei der `toenung`-Zeilen sind jetzt am Referenzbild des Originals kalibriert (`design/look-referenz.md`) statt neutral — Wiesengrund (Zeile 0), Hangmoos (Zeile 11) und der helle raue Fels (Zeile 5, von Beton auf tan-braun). Gerechnet wird dabei NICHT `t = (Ziel/Ist)^2.2`: Vierzig bis vierundsechzig Prozent des Bildwertes hängen gar nicht an der Albedo (Himmelsterm plus Nebel), gemessen am 45°-Hang mit zwei bekannten Tönungen auf denselben Bildpunkten. Aus zwei Messungen fallen `D` (skaliert mit der Albedo) und `A` (skaliert nicht), und daraus `t = (Ziel_linear − A) / D` — mit der naiven Formel wäre die Felszeile bei S 0,26 statt 0,38 steckengeblieben. Zeile 4 (`rock-a`, die dunkle Wandschicht) bleibt bewusst neutral: Bei Metallic 0,85 ist ihr Bild der Himmelsterm, eine Tönung dort bewegt B−R um weniger als ein Byte — nachgemessen an einer 24°-Schwarzwaldflanke, wo diese Schicht überhaupt zu sehen ist (am steilsten Hang zeichnet längst `RAU_TILE`). Zusätzlicher Wächter: `tools/test/look-referenz.ts`. **Triplanar 09.09.2026:** Der Splat projiziert die drei Hangschichten jetzt aus drei Richtungen statt nur von oben, sobald die Neigung 25° übersteigt (`TRIPLANAR`, `triplanarGewichte` in `TerrainSplat.ts`) — vorher zog sich ein Texelstreifen über jede senkrechte Wand, die eine Geländebearbeitung stehen lässt. Der Test prüft die Gewichtsfunktion als reine Funktion: Summe 1, unter der Schwelle EXAKT `[1, 0, 0]` (daran hängt der Nachweis, dass flacher Boden bitgleich bleibt), monoton fallend, und auf der senkrechten Wand kommt nichts mehr von oben. Dazu die umgestellte `RAU_TILE`-Zeile: Schwarzwald und Sumpf tragen auf der Wand jetzt den DUNKLEN Fels (Tile 4, Metallic 0,85) statt des hellen Bergfelses (Tile 5) — geprüft über das Metallic der Schicht, nicht über den Kachelnamen. |
...
| `fix-glb-buffer-length.mjs` | Repariert falsche `buffers[0].byteLength` aus dem Fremdexport. Three toleriert das, Babylon validiert streng. |
| `fix-creature-models.js` | Rettet Kreaturenmodelle aus dem Fremdexport — die Prefab-GLBs haben kein eingebettetes Material. Modellordner `WOV_MODELS_DIR` (Vorgabe `assets/models`). |
| `recover-textures.mjs` | Holt echte Texturen aus dem Client-Export zurück (2.639 von 2.763 PNGs waren 0 Byte). Export-Ordner `WOV_CLIENT_EXPORT`; der frühere Name `VALHEIM_CLIENT` wird weiter gelesen, veraltet. |
| `test-glb-parse.ts` | Lädt GLBs durch threes GLTFLoader wie der Client, um „Model missing" außerhalb des Browsers zu reproduzieren. Modellordner `WOV_MODELS_DIR` (Vorgabe `assets/models`). |
...
| `worldlayout-mcp/` | MCP-Server für das Weltlayout (lesen, setzen, ausrollen). Benutzt `sanitizeWorldLayout` aus `shared`. Schreibt die Weltdatei **nicht selbst**, sondern über den Betriebsdienst (`GET`/`POST /api/worldlayout`, Basis-Hash, `409` bei fremder Änderung). Adresse: `WOV_ADMIN_URL` bzw. `WOV_ADMIN_ADRESSE`/`WOV_ADMIN_PORT`, Vorgabe `127.0.0.1:2468`. Token: `WOV_ADMIN_TOKEN`, sonst `WOV_ADMIN_TOKEN_DATEI`, sonst die Datei `admin.token` im Ordner `server/data/` des Checkouts (legt `npm run dev` an), sonst `/etc/wov-admin.token` (wov-dev). `.mcp.json` braucht kein Geheimnis; Einzelheiten in `Docs/10-Weltbau-Layout-und-Editor.md`. Schreibt nur, wenn der Betriebsdienst die Weltdatei **dieses Checkouts** verwaltet (`weltKennung` aus dem GET; sonst Verweigerung, Lesen bleibt frei; `WOV_MCP_FREMDE_WELT=1` hebt es bewusst auf), damit ein Agent im Worktree nicht per Vorgabe in die DEV-Welt schreibt. Ein Symlink auf der eigenen Weltdatei, der aus dem Checkout hinauszeigt, gilt nicht als eigene Welt; `WOV_WURZEL` liest der Server nicht. Zum Erproben einen eigenen Betriebsdienst auf dem Slot-Port starten (`probe.ts` macht das). **Fußangel in `.mcp.json`:** `${VAR}` ohne Vorgabe (`${VAR:-…}`) bleibt bei nicht gesetzter Variable **wörtlich** stehen (Claude Code 2.1.273 gemessen); eine `WOV_ADMIN_TOKEN`-Zeile ohne Vorgabe schickte den Text `${WOV_ADMIN_TOKEN}` als Token. Deshalb trägt die Datei nur Einträge mit Vorgabe. |
...
- `scan-missing-models.mts` liest den Modellordner aus `WOV_MODELS_DIR`
  (Vorgabe `assets/models`). Die Prefab-Tabelle nennt noch viele Fremdmodelle,
  die es nicht mehr gibt; gegen den DEV-Bestand (209 GLBs) meldet das Werkzeug
  4.128 von 4.305 Modellen als fehlend.
