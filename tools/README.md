...
| `store-prefabs.mjs` | Erzeugt `shared/src/storePrefabs.ts` aus `assets/store/manifest.json` und `assets/store/prefabs.json` — die Asset-Brücke: 569 `PrefabDef` (Name = `id` aus prefabs.json, `model` = `store/<pfad>` bzw. `store-lab/<pfad>` für Bauer Bs abgeleitete Vegetation, `renderScale` aus der Hüllbox, Flag `PERSISTENT`) plus `STORE_KATALOG` mit ALLEN 670 vorhandenen Dateien — auch Texturen, Töne und Kollisionsnetze — sauber einsortiert nach Art/Gruppe/Untergruppe. Die Einsortierregel steht NICHT hier, sondern in `einsortieren()` in `client/src/editor/StoreKatalogDaten.ts` — dieselbe, mit der der Gegenstands-Katalog seine Schubladen baut; bis zur Zusammenführung gab es sie zweimal, und die beiden widersprachen sich bei 477 der 670 Einträge. Deshalb läuft der Generator mit **`tsx`**, nicht mit `node`. Wächter: `tools/test/store-einsortierung.ts`. Deterministisch und idempotent: zweiter Lauf byteidentisch, kein Zeitstempel. `--pruefen` schreibt nichts und endet mit Code 1, wenn die eingecheckte Datei veraltet ist; `--nach <pfad>` schreibt woandershin (nutzt `tools/test/store-erzeugung.ts`, damit ein Test keine getrackte Quelldatei anfasst). Typen und der Spiegelungs-Schalter stehen von Hand in `shared/src/storeKatalog.ts`. Ohne `assets/store` bricht der Lauf mit Code 2 ab. |
