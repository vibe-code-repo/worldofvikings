...
const [directory, validatorPath, canonicalFolder = 'game-ready'] = process.argv.slice(2);
assert(directory && validatorPath, 'Expected asset-directory validator-package-path');
assert(/^[a-z][a-z0-9-]*$/.test(canonicalFolder), 'Expected a safe canonical subdirectory name');
const validator = require(path.resolve(validatorPath));
(async () => {
  const report = {};
  for (const folder of [directory, path.join(directory, canonicalFolder)]) {
    for (const file of fs.readdirSync(folder).filter(f => f.endsWith('.glb'))) {
      ...
      if (result.issues.numErrors || (folder !== directory && result.issues.numWarnings)) process.exitCode = 1;
    }
  }
