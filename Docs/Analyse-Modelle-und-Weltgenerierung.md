...
**Quellen:** der erste Prototyp (Browser-Game), dessen Asset-Ordner und der C++-Referenzserver
...
5. **Die gute Nachricht**: Der C++-Referenzserver enthält die komplette Original-Weltgenerierung (1:1-Port des WorldGenerator, Game-Version 0.221.10, WorldGen-Version 2) — inklusive aller Zahlenwerte, Unity-exaktem Perlin, RNG und geparsten Datenpaketen (vegetation.pkg, features.pkg, prefabs.pkg). Das ist eine vollständige Port-Blaupause.
...
- Verifiziert: der `models/`-Ordner des Prototyps (7463 GLBs) ist eine 1:1-Kopie von `tools/assetripper/export/Assets/PrefabHierarchyObject/` (MD5-geprüft bei FirTree.glb).
...
# Teil 2: C++-Weltgenerierung (Referenzserver)

Analysiertes Repo: der C++-Referenzserver — Port des originalen WorldGenerator, Ziel-Game-Version **0.221.10** (`library/include/CompileSettings.h:362-363`), WorldGen-Version **2** (Z. 372). Die `.pkg`-Daten wurden vollständig binär geparst.
...
**C6-Verifikation** (Seed `KxSYuZquuw`, WorldGen v2, server.yml-Flags): Neues, rein additives Export-Tool für den Geo-Export im C++-Repo (`library/test/geo/`, **keine Änderung an Server-Sourcen**) schreibt `geo_structure.csv` / `geo_riverpoints.csv` / `geo_samples.csv`; TS-Harness `shared/test/geo-compare.ts` vergleicht bitgenau (ULP-Distanz auf f32-Bitmustern):
...
**Flag mit C++-Parität an:** `experimental-ashlands-modern-noise: true` ist C++-Default (Referenzserver + dessen server.yml) — jetzt auch bei uns (DEFAULT_CONFIG true, main.ts `?? true`, server.yml-Schlüssel ergänzt); der bisherige Start-Downgrade auf Legacy ist entfernt.
