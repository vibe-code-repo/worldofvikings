/**
 * GeoManager — 1:1 port of the reference geo manager.
 *
 ...
 * Everything below follows the reference implementation statement by
 * statement.
 *
 * PRECISION MODEL (matches the reference HEIGHTFIX-02 state):
 *  - PerlinNoise and all height-function INTERMEDIATES: double (plain JS
 *    numbers) — bit-exact.
 *  - 2D vector geometry (rivers/streams/lakes), magnitude(), WorldAngle,
 *    RNG draws, Biome-noise ARGUMENTS: float32 — emulated with Math.fround
 *    at exactly the operations where the reference uses `float`.
 *  - Function RESULTS declared `float` in the reference: fround at the return.
 *  - Trig: the reference uses single-precision sin/cos/atan2; Math.* + fround
 *    can differ by ~1 ulp. This can in principle flip a borderline comparison (a river end
 ...
 * legacy/pregen formulas, exactly like the reference.
 */
...
/** Round to nearest float32 (IEEE single), like a `float` cast/store. */
...
/** World size, a 32-bit INTEGER constant (10000). */
export const WORLD_SIZE = 10000;
/** Water edge, a float constant (10500). */
...
/** Zone edge length in metres (used by getBiomeArea/getBiomes). */
...
/**
 * River point: position p, width w and squared width w2 = w*w (float32).
...
export interface GeoManagerSettings {
  /**
   * World generation version. Affects minMountainDistance (<=0: 1500),
   * minDarklandNoise / maxMarshDistance (<=1: 0.5 / 8000). Default 2.
   */
  worldGenVersion?: number;
  /** Disables distant rivers (default false). */
  disableDistantRivers?: boolean;
  /** Rivers also affect the ocean (default false). */
  riverAffectsOcean?: boolean;
  /**
   * Modern Ashlands noise (default true). Uses the
...
/**
 * The geo manager. Constructing runs postWorldInit() + generate()
 * (lakes → rivers → streams), exactly like the reference server at world load.
 */
export class GeoManager {
  ...
  // Cached river grid cell / its river points (1-entry memo; pure perf).
  ...
  // Version-dependent (non-constant members mutated by postWorldInit)
  private minMountainDistance = 1000;
  private minDarklandNoise = f32(0.4);
  private maxMarshDistance = 6000;

  /**
   * Noise generator — created in postWorldInit only when
   * ashlandsModernNoise is on, with the reference client's settings:
   * seed=0 (NOT the world seed!), frequency=0.01f
   * (f32 widened to double), octaves=2, FBM, Euclidean/Distance,
   * jitter=0.45f (float!). Null in legacy mode.
   */
  ...
  /**
   * @param worldSeed Stable hash of the seed name (int32).
   */
  constructor(
    ...
    // postWorldInit: noise generator init
    if (this.settings.ashlandsModernNoise) {
      this.fastNoise = new FastNoise(0); // client uses seed=0, NOT world seed!
      this.fastNoise.setFrequency(f32(0.01)); // frequency 0.01f — f32 widened
      this.fastNoise.setFractalOctaves(2);
      // The FBM fractal type, Euclidean, Distance and jitter 0.45f are the
  ...
  /** Stats for tests/diagnostics (not in the reference). */
  ...
  /** C6 test access — private in the reference (offsets are ints held in float). */
  ...
  private postWorldInit(): void {
    ...
    // The reference calls state.range(-worldSize, worldSize) with worldSize
    // being a 32-bit int — the INT32 overload wins, so offsets are INTEGERS in
    // [-10000, 9999] (not floats like in the original). Reproduced deliberately.
  ...
  /**
   * Runs lake, river and stream generation.
   *
  ...
  private generateLakes(): void {
    ...
    for (let num = -WORLD_SIZE; num <= WORLD_SIZE; num += 128) {
      for (let num2 = -WORLD_SIZE; num2 <= WORLD_SIZE; num2 += 128) {
        // The reference compares against the FLOAT literal 0.05f — using the double
  ...
  /** Merges points closer than `range` into their midpoints. */
  private mergePoints(points: V2[], range: number): V2[] {
    const list: V2[] = [];
    while (points.length > 0) {
      let vector = points[0];
      points.shift(); // erase from the front — O(n), as the reference notes
      while (points.length > 0) {
  ...
  /** Index of the closest point within `maxDistance` of `p`, or -1. */
  ...
  private generateStreams(): void {
    ...
    for (let i = 0; i < STREAMS; i++) {
      ...
      if (start === null) continue; // short-circuit: no end-point search
  ...
  /**
   * Finds a stream start point (with its height) in the height band
   * (minHeight, maxHeight) — returns the start point or null.
   */
  private findStreamStartPoint(
    ...
    for (let i = 0; i < iterations; i++) {
      // The reference passes floats to range() — FLOAT overload
  ...
  /**
   * Finds a stream end point between minLength and maxLength from `start`
   * in the height band (minHeight, maxHeight) — returns end or null.
   */
  private findStreamEndPoint(
    ...
    for (let i = 0; i < iterations; i++) {
      ...
      // start + (sin(f), cos(f)) * num2  (x=sin, y=cos!)
  ...
  private generateRivers(): void {
    ...
    while (list2.length > 1) {
      ...
      // lake can source multiple rivers). Exact reference control flow.
      ...
      if (num !== -1) {
        ...
        // Draw order: widthMax, then widthMin (RNG sequence matters!)
  ...
  /** Picks a random valid river end among `points`, or -1. */
  ...
  /** Whether a river between p0 and p1 is allowed (sampled every `step`). */
  private isRiverAllowed(p0: V2, p1: V2, step: number, heightLimit: number): boolean {
    ...
    for (let num2 = step; num2 <= limit; num2 = f32(num2 + step)) {
      ...
      // float literal 0.05f (see generateLakes note)
      if (baseHeight > f32(0.05)) flag = false;
    }
    return !flag;
  }

  /** Renders rivers into the grid of river points. */
  ...
  /** Adds a river point to every grid cell it reaches — grid fan-out. */
  ...
  /** Whether the river point (p, r) reaches into grid cell (gx, gy). */
  ...
  /** River grid cell of a world position — float32 intermediate. */
  ...
  /**
   * River weight and width at a world position.
   * Includes the 1-entry grid cache (pure memoization).
   * (Private in the reference; public here for the C6 comparison harness.)
   */
  ...
  /** River weight and width from the river points of one grid cell. */
  private getWeight(points: RiverPoint[], wx: number, wy: number): { weight: number; width: number } {
    ...
    for (const rp of points) {
      // squared distance to the river point — float32
  ...
  /**
   * World angle:
   *   return sin(atan2(wx, wy) * 20);   // NOTE: atan2(x, y)!
   * float32 (single-precision atan2/sin approximated with fround — ~1 ulp risk).
   */
  ...
  /**
   * Base height — HEIGHTFIX-02 double
  ...
  /**
   * Base-height tilt — slope estimate used by
  ...
  /** Marsh height — NOTE: no seed offset! */
  ...
  /** Forest height (also Mistlands-pregen). */
  ...
  /**
   * Mistlands height — terraced.
   * Returns { height, mask } (the reference returns the mask via an out-param).
   */
  ...
  /**
   * Ashlands height —
   * LEGACY branch only (ashlandsModernNoise=false). The modern
  ...
  private getAshlandsHeightLegacy(wx: number, wy: number): { height: number; mask: number } {
    ...
    const wy2 = wy;
    let num = this.getBaseHeight(wx, wy);
    // wx += 100000.f + offset3  (float32 adds)
    wx = f32(wx + f32(100000 + this.offset3));
    wy = f32(wy + f32(100000 + this.offset3));
    // num2 is declared `float` in the reference — the double Perlin product is rounded
  ...
  /**
   * Ashlands height —
   * MODERN branch (ashlandsModernNoise=true, ASHLANDS_2.0).
   * Double-precision intermediates with explicit
   * float32 casts exactly where the reference has them (magnitude args/results,
  ...
  private getAshlandsHeightModern(
    ...
    if (!this.fastNoise) {
      // Unreachable (dispatch checks the flag); mirrors the reference's
      // missing-noise-generator guard, which falls back to legacy.
    ...
    // The client uses the Mathf-like smooth step (NOT SmoothStep)
    ...
    // (double)(100000.f + offset3) — the inner add is float32
    ...
    // Lava mask base (fbm is the float32 variant)
  ...
  /**
   * Biome classification of a world position.
  ...
  getBiome(wx: number, wy: number): Biome {
    ...
    // NOTE: 'ashlandsMinDistance + num' is a FLOAT32 addition in the reference
    // (float constants) — the f32 rounding can flip borderline
    ...
    // Biome-noise ARGUMENTS are computed in float32 in the reference (float exprs
  ...
  /**
   * Biome of a zone from five samples.
   *
   * ⚠ QUIRK REPLICATED ON PURPOSE: the reference combines the five
   * samples with the LOGICAL operator `||` (not the bitwise `|` of the
   * original code):
   *   return Biome(getBiome(...) || getBiome(...) || ...);
   ...
   * reviewing in the reference — likely a typo.)
   */
   ...
   * getBiomes() returns. Provided for future use (e.g. if the
   * reference server fixes the `||` typo); the 1:1 port above is the reference.
   */
  ...
  getBiomeArea(x: number, z: number): BiomeArea {
    ...
    // The reference computes point - (±U, 0, ±U) — note the flipped signs:
  ...
  /**
   * Final terrain height in meters (biome height × 200).
   */
  ...
  /**
   * Terrain height plus mask output.
   * mask is only meaningful for Mistlands/AshLands (0 otherwise; the reference leaves
  ...
  /**
   * Height of a biome at a position (with mask output and preGeneration flag).
    ...
    switch (biome) {
      ...
      case Biome.AshLands:
        if (preGeneration) {
          ...
          // intermediates (HEIGHTFIX-02), matches the client's
          // pregeneration height.
          let num = this.getBaseHeight(wx, wy);
          const dwx = wx + 100000.0 + this.offset3;
          const dwy = wy + 100000.0 + this.offset3;
          let num2 =
   ...
   * (The position is scaled by 0.01f then by d — float32 per component; Fbm
   * uses x and z.)
   */
  ...
  /**
   * In-forest test: forest factor < 1.15f —
  ...
  /**
   * Terrain delta — samples 10 random points in the circle, returns the
   * height spread and the downhill direction.
   *
   * ⚠ QUIRK REPLICATED: num2 (the MAX tracker) is initialized with
  ...
  getTerrainDelta(
    ...
    const num = 10;
    let num2 = 1.1754943508222875e-38; // FLT_MIN
    let num3 = FLT_MAX;
    let b = center;
    let a = center;
    for (let i = 0; i < num; i++) {
      // vector = inside-unit-circle sample * radius;  (f32)
      const v = state.insideUnitCircle();
      const vx = f32(v.x * radius);
      const vy = f32(v.y * radius);
      // vector2 = center + (vector.x, 0, vector.y); (f32 adds)
    ...
    // slopeDirection = normalize(a - b) — 3D, but y is always 0,
