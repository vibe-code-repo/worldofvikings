"""The Emberrage design: charcoal steel, oxblood wool, red fractures and bindrunes.

design(ctx) adds its geometry to ctx.pieces[<region>] with the scaffold's helpers.
after_export(ctx) records the set's glow in validation.json. The geometry is the one
the set shipped with.
"""
import json
import math
import bpy
...
def design(ctx):
    VARIANT, rig, pieces = ctx.VARIANT, ctx.rig, ctx.pieces
...
def after_export(ctx):
    ...
    ctx.report['vfx'] = ctx.equipment['vfx']
    (ctx.ROOT/'validation.json').write_text(json.dumps(ctx.report, indent=2)+'\n')
