 ...
 *
 * ── Ports ──────────────────────────────────────────────────────────
 * Eigener Serverstart je Unterfall (s. Begruendung bei f1-truhe.ts: eigene
 * WORLDS_DIR pro Lauf statt eines gemeinsamen Servers haelt die fuenf
 * Szenarien unabhaengig voneinander). Jeder Server bekommt seinen Port vom
 * Betriebssystem (`port: 0`, danach `portVon(server)`, scripts/testport.mjs).
 *
...
async function testSichtbarkeit(): Promise<void> {
  ...
  const server = createWovServer({ port: 0, everyoneAdmin: true, worldsDir: WORLDS_DIR, kontenDir: resolve(WORLDS_DIR, 'konten'), worldName: 'g3-sichtbarkeit', saveIntervalMs: 3600_000 });
  server.start();
...
async function testChatReichweite(): Promise<void> {
  ...
  // nicht die Rechtevergabe — deshalb hier ausdruecklich erlaubt, statt
  // sich still auf eine Sicherheitseinstellung zu stuetzen.
  const server = createWovServer({ port: 0, everyoneAdmin: true, worldsDir: WORLDS_DIR, kontenDir: resolve(WORLDS_DIR, 'konten'), worldName: 'g3-chat', saveIntervalMs: 3600_000 });
  server.start();
  const PORT = portVon(server);
  try {
...
async function testGleichzeitigesBauen(): Promise<void> {
  ...
  // nicht die Rechtevergabe — deshalb hier ausdruecklich erlaubt, statt
  // sich still auf eine Sicherheitseinstellung zu stuetzen.
  const server = createWovServer({ port: 0, everyoneAdmin: true, worldsDir: WORLDS_DIR, kontenDir: resolve(WORLDS_DIR, 'konten'), worldName: 'g3-bauen', saveIntervalMs: 3600_000 });
  server.start();
  const PORT = portVon(server);
  try {
...
async function testTrucheZuZweit(): Promise<void> {
  ...
  // nicht die Rechtevergabe — deshalb hier ausdruecklich erlaubt, statt
  // sich still auf eine Sicherheitseinstellung zu stuetzen.
  const server = createWovServer({ port: 0, everyoneAdmin: true, worldsDir: WORLDS_DIR, kontenDir: resolve(WORLDS_DIR, 'konten'), worldName: 'g3-truhe', saveIntervalMs: 3600_000 });
  server.start();
  const PORT = portVon(server);
  try {
...
async function testPeerGeht(): Promise<void> {
  ...
  // nicht die Rechtevergabe — deshalb hier ausdruecklich erlaubt, statt
  // sich still auf eine Sicherheitseinstellung zu stuetzen.
  const server = createWovServer({ port: 0, everyoneAdmin: true, worldsDir: WORLDS_DIR, kontenDir: resolve(WORLDS_DIR, 'konten'), worldName: 'g3-peer-geht', saveIntervalMs: 3600_000 });
  server.start();
  const PORT = portVon(server);
  try {
