...
if (roots.length === 0) {
  ...
  console.error('  e.g. node tools/dump-envsetup.mjs tools/asset-export/export');
  process.exit(2);
}
...
if (found.length === 0) {
  console.error(
    '[dump-envsetup] no EnvSetup assets found.\n' +
      ...
      '  (with or without Unity\'s m_ prefix). Check that the asset export\n' +
...
const payload = {
  comment:
    'Environment lighting records, written by tools/dump-envsetup.mjs. ' +
    'Data for shared/src/environment.ts — do not hand-edit.',
  count: envs.length,
  environments: envs,
};
