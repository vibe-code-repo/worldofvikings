...
| Datei | Zielpfad | Zweck |
|---|---|---|
| `felsnetz.py` | `tools/elements/blender/felsnetz.py` | Erzeugt: die Fels-Frontschicht **aus dem gescannten Netz** (Quelle `mesh`, `make-stonevault.py --relief-quelle <datei.glb>`). Weg 2 vom 05.09.2026: Der Kontaktbogen `out/kontaktbogen-tripo-vs-wand.png` hat gezeigt, dass ein Höhenfeld auf einem 6,25-cm-Raster keine Rundung darstellen kann, die feiner ist als seine Masche — die Wand las sich als zersplittertes Glas, das Modell als runde Brocken. Hier ist die sichtbare Fläche das Netz selbst: der Scan wird gekachelt (1,70 m je Kachel, je Kachel andere Lage aus der Diedergruppe, Versatz, ±10 % Skalierung, Tiefenstaffelung), durch EINEN exakten Booleschen Schnitt mit Selbstverschneidung verschmolzen, an sechs Ebenen auf die Paneelfläche beschnitten, auf das Budget dezimiert und mit einer Cavity aus der Netzkrümmung versehen. Der RAHMEN bleibt der des Rezepts — Rückplatte, Sockel, Haube, Endstreifen, Randniveau, `_col`, x-Negation. Braucht `bpy`; die Zusagen misst deshalb `fels-frontschicht.mjs` an den ausgelieferten GLB statt am Feld. |
| `fels-frontschicht.mjs` | `tools/elements/pruefung/fels-frontschicht.mjs` | Prüft: das Höhenfeld der Fels-Frontschicht (`--stil fels`) — Naht, Hub, Budget. Hält die Zusagen von F3 fest: die Randspalte des einen Moduls trifft die des Nachbarn in Höhe UND Tiefe (auch über die drei Wandvarianten hinweg), in jedem gebauten Ausschnitt steht ein Punkt ganz vorn und keiner weiter (die Hüllbox muss gleich bleiben, weil `DG_RockVault` abgeleitet wird), der Hub trägt mindestens 12 cm und lässt 1,5 cm Wandstärke stehen (die alte Kapselgrenze von 10 cm ist mit Mass A vom 05.09.2026 weggefallen, s. `fels-kollision.mjs`), und ein Wandpaneel bleibt unter 4500 Dreiecken. Text und Arithmetik, kein `assets/`, kein Blender — hängt in `scripts/run-tests.mjs` hinter einer `python3`-Weiche. Seit Weg 2 (05.09.2026) kommt ein zweiter Abschnitt dazu, der die AUSGELIEFERTEN Wandpaneele misst: Die Quelle `mesh` (`felsnetz.py`) hat kein Feld, ihre drei Zusagen — dicht (keine Kante mit nur einer Fläche), Rand exakt auf dem Randniveau, Dreiecksbudget — sind nur an der Datei prüfbar. |
| `fels-kollision.mjs` | `tools/elements/pruefung/fels-kollision.mjs` | Prüft: dass jedes Fels-Wandmodul ein `_col`-Netz hat — und dass es glatt ist. Mass A vom 05.09.2026: Die Frontschicht darf 18 statt 9 cm tief sein, weil die Spielerkapsel an einer glatten Kollisionsfläche entlanggleitet statt am Relief. Ein fehlendes `_col` hat kein sichtbares Symptom, nur eine Figur, die irgendwann in einer Kluft hängt — deshalb misst dieser Prüfer die ausgelieferten GLB (Blender headless) auf Vorhandensein, Materialslot `Kollision`, deckungsgleiche Hüllbox, grösseres Volumen (er füllt die Klüfte) und ≤ 600 Dreiecke. Zelle, Säle und das Ziegelkit dürfen keins haben. |
| `fels-cavity.mjs` | `tools/elements/pruefung/fels-cavity.mjs` | Prüft: die gebackene Verschattung (`COLOR_0`) in den Fels-Modulen. Mass D vom 05.09.2026: In der Krypta steht kein gerichtetes Licht — der Relieffaktor σ/µ mit gegen ohne Normal-Kanal war 1,001, das Relief war da und zeigte sich nur nicht. Die Wandmodule tragen deshalb je Stützstelle einen Faktor 0,55…1,0 aus der Krümmung des Höhenfeldes (`felsrelief.py`, `_cavity`) im Netz, den das Steinmaterial aufs Albedo multipliziert. Geprüft werden Vorhandensein, Wertefenster, **Streuung** (eine Spalte aus lauter Einsen wäre vorhanden, gültig und wirkungslos — das ist der wahrscheinlichste stille Ausfall), die Alphaspalte (VEC4 allein schiebt Babylon sonst das ganze Kit in den Alpha-Blend-Pfad) sowie dass `_col`-Netze, Zelle, Säle und das Ziegelkit KEINE Farbe tragen. Liest die GLB mit `node` allein — kein Blender, kein Python. |
| `relief-gradient.py` | `tools/elements/pruefung/relief-gradient.py` | Prüft: wie grob die Fels-Frontschicht wirklich ist — Reliefspanne und Neigungshistogramm des Höhenfeldes. Die Zahl zu Mikes Befund „sieht viel weniger grob aus als mein Tripo-Modell“: gemessen wird das FELD über acht Feldschlüssel, nicht ein Bild. Der Kopf trägt die vier Stände vom 05.09.2026 (7,5 cm/4,4° → 16,5 cm/16,3°). Kein Blender, kein `assets/`. |
| `render-felsvergleich.py` | `tools/elements/pruefung/render-felsvergleich.py` | Prüft: die Fels-Frontschicht im STREIFLICHT — der Kontaktbogen zu F3. Zwei Betriebsarten: `--vergleich` stellt je ein Paneel aus mehreren Ordnern nebeneinander (Ziegel / bisher / neu), `--reihe` setzt vier Paneele Kante an Kante — nur so sieht man, ob sich das Feld alle 2 m wiederholt und ob die Naht hält. Ungetextetes Grau und 20-Grad-Einfall: Es soll die GEOMETRIE beurteilt werden, nicht die Steintextur. Bilder nach `out/` (wird beim Lauf erzeugt, nicht committet). |
| `relief-kontrast.mjs` | `tools/elements/pruefung/relief-kontrast.mjs` | Prüft: die Helligkeitsstreuung einer Wand im Streiflicht, mit und ohne Normal-Kanal (`?relief=1` gegen `?relief=0`) — die Messung zu F1. Seit Mass D (05.09.2026) mit `--kanal=cavity` auch die gebackene Verschattung (ausgelieferte Vorgabe gegen `?cavity=0`); die wirkt OHNE Licht und wird deshalb im Grab `hell-probe` ohne Fackel gemessen. Dabei stellt die Sonde sich SELBST vor die Reliefseite eines Wandpaneels (`WOV_CAVITY_WAND/INSTANZ/ABSTAND`) — am Eingangspunkt füllt die glatte Rückseite eines versiegelnden Paneels das Bild, und deren COLOR_0 ist überall 1,0; genau daran ist am 05.09.2026 ein Tag verlorengegangen. Zeugen sind `#define VERTEXCOLOR` im übersetzten Shader und die gemeldete Streuung des COLOR_0 der Messwand. Läuft **lokal** gegen den Tunnel, nicht auf `wov-dev`. |
