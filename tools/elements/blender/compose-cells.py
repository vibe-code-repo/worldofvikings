# Erzeugt: das M1-Nachweisbild aus Zellen — Gang plus Seitenraum, Innenblick mit 20 mm Brennweite.
...
cam_d = bpy.data.cameras.new("cam"); cam_d.lens = 20
cam = bpy.data.objects.new("cam", cam_d); bpy.context.scene.collection.objects.link(cam)
bpy.context.scene.camera = cam
cam.location = yup(0, 1.8, -0.4)
cam.rotation_euler = (yup(0, 1.3, 7) - cam.location).to_track_quat("-Z", "Y").to_euler()
...
w.use_nodes = True; w.node_tree.nodes["Background"].inputs[1].default_value = 0.14
