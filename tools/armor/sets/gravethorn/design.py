"""The Gravethorn design: grave steel with bone rims that end in flame thorns.

Heavy grey-green plate whose pale bone rims run out into flame-shaped thorns. Closed
raven-beak helm with blade horns and a centre comb, stacked pauldrons that carry a wolf
skull and an upright blade, red gems on chest, belt and back, a split plate skirt and a
long back banner. Every glowing part (eyes, gems, skull eyes, horn and skull flames) is
emissive geometry that belongs to its item.

The shoulders are bound in three steps: crown tier, wolf skull and blade are rigid on
Shoulder_Attachment_L/R, the middle tier is shared with the upper arm, the great tier
follows the upper arm. So a raised arm lifts the lower plates instead of passing through
them, and nothing travels with the forearm. The helm horns are rigid on Head.

design(ctx) adds the geometry to ctx.pieces[<region>], records the socket-bound parts in
ctx.wing_binding_report and the triangle budget in ctx.budget. after_export(ctx) adds the
review images the scaffold does not make (a true side view; front and back in a quick
run) and the glow counts per item to equipment.json and validation.json. The geometry is
the one the set shipped with.
"""
import json
import math
...
EMISSIVE = CONFIG['emission']  # the glowing materials: red, eyes, glow, core
