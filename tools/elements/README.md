...
| `relief-kontrast.mjs` | `tools/elements/pruefung/relief-kontrast.mjs` | Prüft: die Helligkeitsstreuung einer Wand im Streiflicht, mit und ohne Normal-Kanal (`?relief=1` gegen `?relief=0`) — die Messung zu F1. Seit Mass D (05.09.2026) mit `--kanal=cavity` auch die gebackene Verschattung (`?cavity=1` gegen `?cavity=0`); die wirkt OHNE Licht und wird deshalb im Grab `hell-probe` ohne Fackel gemessen. Der Zeuge ist dann `#define VERTEXCOLOR` im übersetzten Shader — er sagt, dass die Farbschicht das Verschmelzen im `AssetManager` überlebt hat. Läuft **lokal** gegen den Tunnel, nicht auf `wov-dev`. |
