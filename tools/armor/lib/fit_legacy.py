"""Fit a set's ornaments and an exact replacement lining to the game's female figure.

    fit(CONFIG, hooks=None)

Blender -b Female/WoV_<Name>_Armor.blend --python sets/<set>/female/fit-legacy.py -- body.glb OUTPUT
(the female authoring blend of the build, and the 51-bone game body). Source bodies and
armor stay read-only; the game body keeps its original skin. The figure is one mesh, so
each item's lining is rebuilt from an exact triangle partition of that body (the game
masks exactly those triangles), the ornaments are fitted with anatomical rest-frame
transforms and their weights moved to the 51-bone rig.

`hooks` is a module with optional functions:
    lining(ctx, slot, original, lining_count, ornament, data)
        gives the rebuilt lining of a region its materials (and may reshape it) before
        it is joined with the ornaments; default: the scaffold's one material per region
    after_fit(ctx)
        runs after the export and equipment.json, for checks or a corrected header
"""
import bpy
import bmesh
import json
...
def fit(config, hooks=None, args=None):
    import sys
    cfg = schema.complete(config)
    NAME = cfg['name']
    PREFIX, LEGACY_PREFIX = 'WoV_'+NAME+'_', 'WoV_'+NAME+'Legacy_'
    ctx = SimpleNamespace(config=cfg, hooks=hooks)
    args = sys.argv[sys.argv.index('--')+1:] if args is None else list(args)
    ctx.body_path, destination = body_path, destination = args[:2]
    ctx.root = root = Path(destination); root.mkdir(parents=True,exist_ok=True)
    ctx.renders = renders = root/'renders'; renders.mkdir(exist_ok=True)
    ctx.scene = scene = bpy.context.scene
    ctx.source_rig = source_rig = bpy.data.objects['WoV_Player_Armature']
    source_rig.data.pose_position='REST'
    ctx.source = source = {o.name.removeprefix(PREFIX):o for o in scene.objects if o.type=='MESH' and o.name.startswith(PREFIX)}
    ctx.source_base = source_base = {s:bpy.data.objects['WoV_BodyBase_Female_'+s] for s in source}
    before=set(bpy.data.objects)
    bpy.ops.import_scene.gltf(filepath=body_path)
    imported=set(bpy.data.objects)-before
    ctx.rig = rig = next(o for o in imported if o.type=='ARMATURE')
    ctx.body = body = next(o for o in imported if o.type=='MESH' and len(o.data.vertices)>1000)
    ...
    ctx.groups = groups = {g.index:g.name for g in body.vertex_groups}
    body.data.calc_loop_triangles()
    # Every triangle of the game body is sorted into a region; the free regions (no item,
    # no lining) get a list too, which is filled and never used.
    ctx.triangles = triangles = {s:[] for s in [*source, *cfg['free_regions']]}
    ...
    ctx.armor = armor = {}
    ctx.report = report = {'targetBody':body_path,'bodyVariant':'female','bodyProfile':'legacy-female-v1','parts':{},'sourceOverwritten':False}
    lining_hook = getattr(hooks, 'lining', None)
    for slot,original in source.items():
        ...
        if lining_hook:
            lining_hook(ctx, slot, original, lining_count, ornament, data)
        else:
            material='black' if slot=='Head' else ('leather' if slot.startswith(('Hand','Leg')) else 'cloth')
            data.materials.append(bpy.data.materials[NAME+'_'+material])
        ...
        obj.name=LEGACY_PREFIX+slot;obj.parent=rig;obj.matrix_world=Matrix.Identity(4)
    ...
    scene.render.resolution_x=scene.render.resolution_y=cfg['fit_render_resolution']
    ...
    cam.location=Vector((1.1,-4,1.5))+offset
    cam.rotation_euler=(Vector((0,0,.59))+offset-cam.location).to_track_quat('-Z','Y').to_euler()
    scene.render.filepath=str(renders/(NAME+'_Legacy_Female_Hero.png'));bpy.ops.render.render(write_still=True)
    ctx.equipment = equipment = json.loads((Path(bpy.data.filepath).parent/'equipment.json').read_text())
    equipment.update({'prefix':LEGACY_PREFIX,'bodyVariant':'female','bodyProfile':'legacy-female-v1','figure':'wikingerin'})
    ...
    export('WoV_'+NAME+'_Legacy_Female.glb',list(armor.values()))
    for part in equipment['parts']: export(part['item']+'.glb',[armor[s] for s in part['regions']])
    bpy.ops.wm.save_as_mainfile(filepath=str(root/('WoV_'+NAME+'_Legacy_Female.blend')))
    ...
    after = getattr(hooks, 'after_fit', None)
    if after:
        after(ctx)
    return ctx
