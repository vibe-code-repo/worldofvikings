 ...
 *      Eintrag, Antwort nennt den Wirksamkeits-Hinweis
...
try {
  ...
  // Der Hinweis sagt seit dem Zusammenfuehren mit lab/adminkonto etwas
  // anderes als vorher: die Datei wirkt OHNE Neustart (AdminListe liest
  // nach), nur eine laufende Sitzung behaelt ihre Rechte. Geprueft wird
  // beides, damit der Text nicht stillschweigend wieder zurueckfaellt.
  check('Hinweis nennt "ohne Neustart"', String(leer.daten.hinweis ?? '').includes('ohne Neustart'));
  check('Hinweis nennt die laufende Sitzung', String(leer.daten.hinweis ?? '').includes('laufenden Sitzung'));
  ...
  check('Wirksamkeits-Hinweis in der Add-Antwort', String(addName.daten.hinweis ?? '').includes('ohne Neustart'));
