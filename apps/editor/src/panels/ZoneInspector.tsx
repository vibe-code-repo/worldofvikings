/**
 * The zone inspector: the ground of the active zone (ADR-0020, ADR-0032,
 * ADR-0033).
 *
 ...
 *
 * **One block, one panel, two commands.** The terrain block is written by two
 * editor commands, and for a while that made it two panels standing next to
 * each other. It is one panel now, in two halves: `Surface` ({@link GroundPanel})
 * turns the dials that go through `updateTerrainSurface` — the command
 * `pnpm terrain-surface` calls — and `Terrain` below it draws everything else
 * from the schema as ordinary field patches. Which field belongs to which half
 * is the *schema's* answer (`turnedBy` in `@wov/world-schema`), so neither half
 * carries a list of field names and no number is offered twice.
 *
...
import { GROUND_COMMAND, GroundPanel } from './GroundPanel.js';
