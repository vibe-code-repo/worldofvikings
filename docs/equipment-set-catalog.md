...
- `tools/armor/test/equipment-set-assets.mjs http://127.0.0.1/`: read-only delivery check for the catalog, one model file and one icon per item currently in `equipmentSetCatalog()` (11 sets, 73 items as of this write). Its PASS line reprints the live count on every run, so read the count there, not here; it grows with every new set or item without a doc edit. Does not log in to a player account.
